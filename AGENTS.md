...
The authenticated loopback wake socket is the immediate desktop-input notification path.
A paired worker connects immediately after loading its credentials, including extension reload;
it must not wait for page traffic or the maintenance alarm before subscribing. If an app
restart closes an established socket, the already-awake extension worker makes bounded fast reconnect
attempts; Chrome's 30-second alarm remains the sleeping-worker fallback rather than the normal send path.

Protected recorder health/repair flights are per tab. An unresponsive protected document must not
block the shared maintenance scan or delivery to other tabs; its own input waits for that repair.
Repeated scans reuse the same outstanding repair rather than issuing duplicate injections.
...
After native Send acceptance, desktop delivery observes the exact canonical user receipt for
up to 120 seconds. The app reserves 180 seconds after authorization for acceptance, receipt and
ACK transport. Expiry preserves uncertainty and never grants another Send click.
Ordinary authored messages and queued edits share the 64,000-character bound in
`shared/input.ts`. The composer preserves oversized pasted drafts and reports the limit
on send; HTML maxlength must not silently truncate them. Prepared transport byte and
aggregate outbox limits still apply independently; planner/Goal limits are separate.

Existing recorded conversations may explicitly inherit the current browser model by sending
null model/effort. A recorded identity absent from the Chat catalog defaults to this inherited
choice; it is not an observed catalog alias. Work model identities must not be mapped to Chat
Pro or have their effort reinterpreted. Display the last recorded identity as historical evidence,
and keep new-chat admission dependent on a confirmed catalog choice.
...
Desktop input offers observe their receipt outside the shared maintenance flight. An in-flight tab/input key suppresses overlapping offers, while durable elections and main-process claims retain send authority. A slow receipt in one chat must not starve a different queued input.
...
Projects may carry user-authored kind/summary/brief/category metadata. The My work collection opens purpose-named cards; work cards and curated sidebar entries select their registered project and prepare an unsent continuation brief. Other folders remain behind a collapsed disclosure and retain the bounded file workspace (`renderer/project-files.ts`, `main/project-files.ts`). No card grants new roots or sends a message automatically.
...
### Malachi Overdrive command centre (custom v2.0.8)

`renderer/command-center.ts` is a local view of the existing renderer snapshot, not a new
authority or data store. Its search is bounded to loaded recordings and must disclose that
coverage. Resolve an action against the latest snapshot at activation; never revive a deleted
or disabled entry from a stale closure. Keep live status buttons mounted while updating their
values so recording notifications cannot discard keyboard focus. Another open modal owns focus.

`renderer/work-recipes.ts` prepares editable text only. Append without changing the person's
existing bytes; reject the complete append at the shared input limit instead of truncating.
Recipe selection must not send input, change Goal/Loop, start workers or widen permissions.
Organised-project navigation restores its current in-memory text/attachment draft. Explicit
New chat remains a fresh draft. Do not describe either behavior as durable across app restarts.

After renderer changes, `npm run build && node scripts/qa-overdrive.cjs` exercises the production
renderer with the real sandboxed preload and isolated fixture IPC. It does not start the
production main process, use the person's profile or contact the browser. Screenshots and
checks go to `artifacts/overdrive-command-center/`; inspect them as well as reading the JSON.
Dialog geometry alone is not enough: preserve the full height of headings, status and controls,
and let only the results pane shrink/scroll. See `docs/overdrive-command-center-handoff.md`.

### Workbench and draft-safety pass — 2026-09-10

`renderer/workbench.ts` projects the existing curated project, loaded session and window-local
draft state. It introduces no store, IPC channel, network request, worker or polling timer.
Keep the view bounded (four projects, three recent chats) and skip unchanged presentation
updates; actions must re-resolve the actual project/session at the chat adapter before navigation.
Draft markers mean in-memory drafts, not restart persistence or a received provider message.

Command results reconcile by command ID instead of rebuilding the list on recording updates.
Keep result DOM IDs, selected identity, scroll and focus intact; remove entries no longer present.
Older-history loading reuses `chat.ts::loadMoreSessions()` with its existing one-page single-flight
guard. Search/open must not drain history automatically or claim unqueried history was searched.

Queued-task editors must validate the shared authored-message limit on Save without HTML maxlength
truncation. Cancellation of an edit is not cancellation of its queued input. An IPC promise rejection
does not prove the backend refused a send: preserve available authored draft state, never clobber a
newer edit, never auto-resend, and keep an accepted send accepted if a later control fails.

`COS_QA_ARTIFACTS=<simple-folder-name> node scripts/qa-overdrive.cjs` isolates new QA output beneath
the repository's artifacts folder; the default destination remains compatible with earlier checks.
Workbench narrow-width QA explicitly collapses the existing sidebar. See
`docs/overdrive-workbench-handoff.md` for this pass's evidence and limits.
