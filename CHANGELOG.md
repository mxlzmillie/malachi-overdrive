...
- Release publication was blocked by the Windows UI Automation fixture readiness gate; no 2.0.21
  GitHub release was created. Its product changes are carried forward into 2.0.22.
...
- Added verified background update staging and safe automatic restart/install for Windows and Linux AppImage builds. The updater waits for a grace period, whole-machine idle, no live browser turn, no in-flight MCP request and no retained terminal process before restarting.
- Bound each packaged client to the GitHub repository that built it. Local builds with no trusted release repository fail closed instead of following an unrelated upstream release channel.
- Made Linux AppImage replacement atomic on the normal same-filesystem path and extended shutdown handoff time for the cross-device fallback.
- Materialize the packaged companion extension into its stable user-data folder on startup so desktop updates refresh the companion bytes without a separate download/re-load step.
- macOS remains manual for this release because the current binaries are publisher-unsigned and unnotarized. Silent Mac replacement stays disabled until MALACHI OVERDRIVE has a stable Developer ID signature and notarized release identity, which is required to avoid repeatedly invalidating Screen Recording/Accessibility trust.

See [the full release notes](docs/release-notes/v2.0.9.md).
