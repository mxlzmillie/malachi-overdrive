...
endpoint and the shared **MALACHI OVERDRIVE Plugins** connector.
...
2. Create the **MALACHI OVERDRIVE Plugins** connector in ChatGPT using the displayed name,
   description and MCP URL. Keep the MALACHI OVERDRIVE connection running.
...
5. A persistent reminder and **Open ChatGPT plugins** action stay visible above the installed list.
   Successful installation, configuration and tool-policy changes also display a reminder to refresh the **MALACHI OVERDRIVE Plugins** connector in ChatGPT. Refresh the connector in ChatGPT after changing enabled tools, or enable MALACHI OVERDRIVE's existing
...
- Pinned npm and Python recipes: Blender MCP, Knowledge Memory, Playwright Browser, Web Fetch and Unity Editor.
  Node.js/npm or Python/uv must be installed where the recipe requires them. MALACHI OVERDRIVE installs
...
is a third-party integration, not an official Blender feature supplied by MALACHI OVERDRIVE.
...
MALACHI OVERDRIVE's approved-folder sandbox. MALACHI OVERDRIVE read-only mode refuses external plugin calls because upstream
...
MALACHI OVERDRIVE remains MIT licensed. Settings provides the bundled Third-party Notices, generated from
...
External plugin licenses are shown per installation and remain separate from MALACHI OVERDRIVE's own license.
