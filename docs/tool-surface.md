...
MALACHI OVERDRIVE publishes Core on Windows, macOS and Linux. Windows and macOS additionally publish
...
| --- | --- | --- |
| **MALACHI OVERDRIVE Core** | Approved files, local web previews, patches, terminal, ChatGPT file saving, recorded-session lookup, workers | `preview`, `read`, `view_image`, `find`, `apply_patch`, `exec_command`, `write_stdin`, `download_artifact`, `session`, `agents` |
| **MALACHI OVERDRIVE Desktop** | **Windows/macOS:** screen, windows, mouse/keyboard and clipboard | `observe`, `computer` |
...
Starts or reuses a static website server for an approved folder containing `index.html`.
The server binds to loopback on an ephemeral port, serves files without caching, and stops
with the app. Its tool result becomes a compact **Open preview** link in the chat timeline,
so a website can be opened without starting or visiting a terminal process.
