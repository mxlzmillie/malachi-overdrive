...
var CLF_DOM = (() => {
   ...
   *
   * MALACHI OVERDRIVE remains authoritative for its own call labels/results. This only supplies
   ...
   * recording "MALACHI OVERDRIVE Desktop is now connected" as a ChatGPT failure.
   */
  ...
  async function pluginRefreshView(connectorName, expectedTools = [], expectedAppId = null) {
    const externalPlugins = connectorName === 'MALACHI OVERDRIVE Plugins';
  ...
  /** A desktop input's pre-Send claim expires after 60s; picker work must finish first. */
  const MODEL_SELECTION_BUDGET_MS = 45000;
  /** UI only transports a requested selection. Provider state proves identity and availability. */
  function modelPickerAccess(stillCurrent, deadline = Number.POSITIVE_INFINITY) {
    ...
    const active = () => stillCurrent() && Date.now() < deadline;
    const trigger = () => {
    ...
    const wait = (read, timeout = 3000) => {
      const remaining = deadline - Date.now();
      if (!active() || remaining <= 0) return Promise.resolve(null);
      return new Promise(resolve => {
        ...
        const check = async () => {
          ...
          reading = true;
          try { let value = read(); if (value?.then) value = await value; if (active() && value) finish(value); }
          catch { finish(null); }
          finally { reading = false; if (dirty && !done) { dirty = false; void check(); } }
        };
        const observer = new MutationObserver(check);
        observer.observe(document.documentElement, { subtree: true, childList: true, attributes: true, characterData: true });
        const timer = setTimeout(() => finish(null), Math.min(timeout, remaining)); void check();
      });
    };
    const state = predicate => wait(async () => { const value = await readPickerState(); return value && (!predicate || predicate(value)) ? value : null; });
    const key = (node, value) => { if (!node || !active()) return false; node.focus(); node.dispatchEvent(new KeyboardEvent('keydown', { key: value, code: value, bubbles: true, cancelable: true })); return true; };
    return {
      state,
      async open() {
      ...
      close() {
        ...
        // Cleanup remains safe after the operation deadline, while page ownership still holds.
        if (!button || !stillCurrent()) return;
        button.focus();
        button.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', code: 'Escape', bubbles: true, cancelable: true }));
      },
  ...
  async function selectModelSettings(model, effort, stillCurrent = () => true) {
    ...
    const deadline = Date.now() + MODEL_SELECTION_BUDGET_MS;
    const selectionCurrent = () => stillCurrent() && Date.now() < deadline;
    const ui = modelPickerAccess(stillCurrent, deadline), original = await ui.open();
    ...
    try {
      ...
      for (const version of [original.versions.find(v => v.id === original.version), ...original.versions.filter(v => v.id !== original.version)]) {
        ...
        selected = selectionCurrent() && confirmed?.available === true && confirmed.id === choice.id && confirmed.effort === choice.effort;
        return selected;
      }
      return false;
    } finally {
