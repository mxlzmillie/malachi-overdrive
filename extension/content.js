...
(() => {
  ...
  // The DOM layer already memoizes an unchanged section's authored text. Keep the derived
  // occurrence signature on the same "id + exact text" boundary so a long historical prompt
  // costs four hashes when it changes, not four hashes on every observer tick. Message ids can
  // be reused by ChatGPT, so the text remains part of the cache identity exactly as it is in
  // the returned key; a changed or reverted body is re-hashed and preserves existing dedupe
  // semantics. Bound this independently of seenMessages so a long-lived tab cannot grow it.
  const occurrenceKeyCache = new Map();

  function occurrenceKey(id, value) {
    const body = String(value || '');
    const cached = occurrenceKeyCache.get(id);
    if (cached && cached.body === body) return cached.key;
    const lanes = [];
    ...
    const key = `${id} :: ${body.length}.${lanes.join('.')}`;
    occurrenceKeyCache.delete(id);
    occurrenceKeyCache.set(id, { body, key });
    if (occurrenceKeyCache.size > MAX_SEEN_MESSAGES) {
      occurrenceKeyCache.delete(occurrenceKeyCache.keys().next().value);
    }
    return key;
  }
  ...
  function emit(observation) {
    ...
    const takeUtf8 = (value, budget) => {
      ...
      const marker = '\n\n[MALACHI OVERDRIVE: browser observation truncated to fit transport.]';
  ...
  /** One pass owns the exact user intervals; no assistant rescans the entire history. */
  function streamAnchorIndex(sourceTurns, groups) {
    const byTurn = new Map();
    let anchor = null;
    for (const turn of sourceTurns) {
      if (turn.role !== 'user') {
        byTurn.set(turn, anchor);
        continue;
      }
      const ids = new Set();
      for (const message of CLF_DOM.messagesIn(turn)) {
      ...
      anchor = ids.size === 1 ? userAnchorByMessage.get(ids.values().next().value) || null : null;
    }
    const sequences = [...new Set([...userAnchorByMessage.values()]
      .map(candidate => Number(candidate && candidate.seq)).filter(Number.isFinite))].sort((a, b) => a - b);
    ...
    for (const group of groups) {
      ...
      if (!Number.isFinite(seq) || !(group.entries || []).some(entry => entry.kind !== 'turn_start' && entry.kind !== 'turn_end')) continue;
      // The strict interval is identical to anchor.seq < start.seq < nextAnchor.seq.
      // A start on an anchor is ambiguous, and is never assigned to either response.
      let low = 0, high = sequences.length;
      while (low < high) {
        const mid = (low + high) >>> 1;
        if (sequences[mid] < seq) low = mid + 1;
        else high = mid;
      }
      if (low === 0 || sequences[low] === seq) continue;
      const key = sequences[low - 1];
      const held = byAnchor.get(key);
      if (held) held.push(group);
      else byAnchor.set(key, [group]);
    }
    return { byTurn, byAnchor, byId };
  }
   ...
   */
  function anchoredRenderForTurn(turn, index, requiredGroup = null, requireRequest = false) {
    const anchor = index.byTurn.get(turn);
    if (!anchor || !Number.isFinite(Number(anchor.seq))) return null;
    const candidates = index.byAnchor.get(Number(anchor.seq)) || [];
   ...
   *
   * Exact names, never a prefix: `MALACHI OVERDRIVE Backup` would be somebody else's
   * connector, and a prefix test would have this app vouch for its traffic.
   */
  const OUR_CONNECTORS = ['MALACHI OVERDRIVE Core', 'MALACHI OVERDRIVE Desktop', 'TobisComputer'];
  ...
  function renderStreams() {
    ...
    // A hidden historical page has no presentation consumer. Its recorder/command paths
    // remain active, but reconstructing every mounted response each second can monopolize
    // the renderer that must answer the next desktop-input message.
    ...
    const viewportAnchor = document?.visibilityState === 'hidden' ? null : presentationViewportAnchor(sourceTurns);
    if (!enabled) {
      for (const root of streamRootsByKey.values()) root.remove();
      streamRootsByKey.clear();
      for (const turn of sourceTurns) {
        if (turn.role !== 'assistant') continue;
        for (const node of turn.nodes || (turn.node ? [turn.node] : [])) {
      ...
      // Repair notices describe app actions independently of the Overwrite preference.
      if (document?.visibilityState !== 'hidden') renderRepairNotices(sourceTurns);
    ...
    const anchors = streamAnchorIndex(sourceTurns, groups);
    ...
    for (let turnIndex = 0; turnIndex < assistantTurns.length; turnIndex++) {
      ...
      const localGroup = localId ? anchors.byId.get(localId) || null : null;
  ...
  function controlState(input) {
    ...
    if (disconnected) {
      return {
        mode: 'off',
        label: 'Compact',
        hint: 'Browser connection is disconnected in MALACHI OVERDRIVE.',
        action: 'none'
      };
    }
    if (!connected) {
      return {
        mode: 'off',
        label: 'Compact',
        hint: 'MALACHI OVERDRIVE is not running on this PC.',
        action: 'none'
      };
    }
    if (!conversationId) {
  ...
  function buildControl() {
    ...
    blocked.setAttribute(
      'data-clf-tip',
      'This chat is blocked in the MALACHI OVERDRIVE app: its tool calls are refused and Goal, Loop and auto-compaction are off. To release it, open the app’s Chat tab, hover this chat in the sessions list and press its block symbol.'
    );
    blocked.hidden = true;
  ...
  function buildMenu() {
    ...
    root.setAttribute('aria-label', 'MALACHI OVERDRIVE settings');
    root.hidden = true;
  ...
  function renderControl() {
    ...
    control.button.setAttribute('aria-label', 'MALACHI OVERDRIVE settings');
  ...
  function replyError(reply) {
    ...
    if (reply.error === 'app_not_found') return 'MALACHI OVERDRIVE is not running on this PC.';
  ...
  */
  ...
  function currentObservationDelay() {
    // Mutations still capture new transcript promptly, including hidden final/Stop edges.
    // The standing tick covers lifecycle settling and missed DOM events, not a requirement
    // to hash and reconstruct an unchanged million-token history once per second forever.
    if (generating || resumeIdentityPending || desktopInputBusy || commandAttempt || nativeBusy ||
        Boolean(job && job.busy) || pendingTools > 0 || Date.now() < fiberSettleUntil || presentationPending()) return OBSERVE_MS;
    return document.visibilityState === 'hidden' ? HIDDEN_ACTIVITY_MS : IDLE_ACTIVITY_MS;
  }

  function periodicObservation() {
    // Keep the cheap wake on the existing clock so a mutation that opens a generation
    // immediately restores the one-second settle clock, even after a long idle interval.
    if (CLF_DOM.conversationId() === conversationId && Date.now() - lastObservationAt < currentObservationDelay()) return;
    observe();
    syncTheme();
    injectControl();
    injectStage();
  ...
  async function acceptDesktopInput(message) {
    ...
    try {
      ...
      // Long submitted bubbles and fresh canonical routes can hydrate well after
      // native acceptance. Keep observing this one send; never click it again.
      }, sendingTarget, 120000);
  ...
  every(OBSERVE_MS, periodicObservation);
  ...
  if (typeof document !== 'undefined' && document.addEventListener) {
    const visibilityChanged = () => {
      ...
      observe();
      // Visibility is presentation demand even if the app feed has not changed. Restore
      // deferred labels/streams synchronously before the next activity round-trip.
      paint();
      renderStreams();
      foldBootstrap();
