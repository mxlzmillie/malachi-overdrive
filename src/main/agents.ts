...
export class IdentityLostError extends AgentError {
  constructor() {
    super(
      'WORKER_IDENTITY_LOST: MALACHI OVERDRIVE could not tell which conversation this call came from, so it cannot ' +
...
function requireEnabled(): void {
  if (!getConfig().multiAgent.enabled) {
    throw new AgentError('Multi-agent mode is switched off in MALACHI OVERDRIVE. Ask the user to enable it.');
  }
}
...
function planRevivalText(agent: Agent): { text: string; messageIds: string[] } {
  ...
  const text =
    ...
    `\n\n(MALACHI OVERDRIVE: you are still ${agent.info.id} in the same run, and this is the prime agent talking to ` +
