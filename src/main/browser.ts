...
export async function isPreferredBrowserRunning(
  ...
  try {
    ...
    const processName = browser === 'edge' ? 'msedge' : browser === 'brave' ? 'brave' : browser === 'opera-air' ? 'opera' : 'chrome';
...
export function preferredBrowserCandidates(
  ...
  if (platform === 'win32') {
    const p = path.win32;
    if (browser === 'opera-air') {
      return [
        env.LOCALAPPDATA ? p.join(env.LOCALAPPDATA, 'Programs', 'Opera Air', 'opera.exe') : '',
        env.ProgramFiles ? p.join(env.ProgramFiles, 'Opera Air', 'opera.exe') : '',
        env['ProgramFiles(x86)'] ? p.join(env['ProgramFiles(x86)'], 'Opera Air', 'opera.exe') : ''
      ].filter(Boolean);
    }
  ...
  if (platform === 'linux') {
    ...
    const names = browser === 'edge' ? ['microsoft-edge', 'microsoft-edge-stable', 'microsoft-edge-beta', 'microsoft-edge-dev'] : browser === 'opera-air' ? ['opera-air', 'opera'] : browser === 'brave' ? [
      'brave-browser',
      'brave-browser-beta',
      'brave-browser-dev',
    ...
    if (browser === 'opera-air') return [...new Set([
      ...fromPath,
      ...names.map(name => path.posix.join('/usr/bin', name)),
      '/opt/opera-air/opera-air'
    ])];
...
export async function openInPreferredBrowser(
  ...
  const label = selected === 'edge' ? 'Microsoft Edge' : selected === 'brave' ? 'Brave Browser' : selected === 'opera-air' ? 'Opera Air' : 'Google Chrome / Chromium';
