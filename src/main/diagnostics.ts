...
export function describeMacOSDesktopAccess(
  ...
  if (caps.control) {
    checks.push(check(
      ...
      'macOS denied AXUIElement access to the in-process MALACHI OVERDRIVE.app backend. Open Privacy & Security → Accessibility (Device Control and Data Access on newer macOS).'
    ));
  }
  return checks;
}
