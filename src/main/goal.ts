...
/**
 * Master execution gate for Goal/Loop.
 *
 * Goal decisions consume the locally recorded transcript. Turning session recording off must
 * therefore suspend execution without erasing the user's saved per-chat switch or objective.
 * Callers use this at the final authority boundary before projecting or starting Goal work.
 */
export function goalRuntimeAvailable(): boolean {
...
export async function retryGoalBrowserHelper(sourceSessionId: string, inputId: string): Promise<boolean> {
  ...
  if (draft && goalRuntimeAvailable() && drafts.get(session.conversationId) === draft &&
      (await getSession(sourceSessionId))?.conversationId === session.conversationId && goalArmedFor(session.conversationId)) {
...
export function beginGoalDraft(conversationId: string, token: string): boolean {
  const draft = drafts.get(conversationId);
  if (!draft || draft.token !== token || draft.acknowledged) return false;
  // Retried bridge POSTs deliberately receive the same reserved token. Once provider work has
  // started, "begin" is already satisfied rather than an error; treating this as false makes
  // the bridge confuse an idempotent retry with revoked runtime authority and return 409.
  if (draft.work) return true;
...
export async function draftOpeningMessage(
  ...
  if (!goalRuntimeAvailable()) return { error: 'goal_disabled' };
  const settings = getConfig().goal;
  const endpoint = goalEndpoint();
