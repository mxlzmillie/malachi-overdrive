...
export function registerIpc(getWindow: () => BrowserWindow | null, quitToInstall: () => void): void {
  ...
  handle('settings:save', async (payload) => {
    ...
    if (
      // Recording is Goal's transcript authority. A per-chat switch can intentionally remain
      // stored while the recorder is Off, so relying on the app-wide Goal switch changing here
      // leaves an already-running per-chat provider request alive past the privacy boundary.
      // Retire it at the settings transaction that withdraws that authority instead.
      (before.sessions.record && !next.sessions.record) ||
  ...
  handle('projects:files', async (payload) => {
    const { id } = z.object({ id: z.string().uuid() }).parse(payload);
    return projectFiles(id);
  });
  handle('projects:preview', async (payload) => {
    const { id, path } = z.object({ id: z.string().uuid(), path: z.string().min(1).max(4096) }).parse(payload);
    return projectFilePreview(id, path);
  });
  ...
  handle('roots:rename', async (payload) => {
    ...
    if (RESERVED_ROOT_NAMES.has(newName)) {
      throw new SandboxError(`/${newName} is reserved by MALACHI OVERDRIVE and cannot be used as a folder name`);
    }
    await updateConfig((config) => {
      if (!config.roots.some((root) => root.name === name)) throw new Error(`/${name} is not an approved folder`);
  ...
  handle('sessions:editInput', async (payload) => { const { id, text, afterTurn } = z.object({ id: z.string().uuid(), text: inputTextSchema, afterTurn: z.boolean().optional() }).parse(payload); return editQueuedInput(id, text, afterTurn); });
  ...
  configureInputDelivery({
    ...
    prepareText: (entry) => {
      const config = getConfig();
      const control = entry.conversationId ? goalSwitchFor(entry.conversationId) : config.goal;
      const mode = entry.automation ?? (control.enabled ? control.mode : 'off');
      const offlineGoal = mode === 'goal' && goalBackendFor('goal') === 'templates';
      return codingAgentDeliveryText(entry.text, {
        standingInstructions: config.mcp.instructions.slice(0, MAX_MCP_INSTRUCTIONS_CHARS),
        terminalInstruction: offlineGoal ? GOAL_MARKER_INSTRUCTION : undefined
      });
    },
    applyAutomation: async (conversationId, automation, phase, objective) => {
      ...
      if (phase === 'after-send' && goalRuntimeAvailable() && held.enabled && live.enabled && live.mode === held.mode) {
        await setGoalReplyActiveNow(conversationId, true);
      }
    }
  });
  ...
  handle('sessions:goalOpening', async payload => {
    ...
    return runTaskRequest(requestId, JSON.stringify(['goal', text, mode]), async signal => {
      ...
      if (!goalRuntimeAvailable()) throw nativeGoalFailure('goal_disabled', backend);
