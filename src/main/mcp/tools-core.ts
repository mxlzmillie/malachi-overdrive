 ...
 *
 * Ten tools at the absolute maximum, and usually eight. That number is the design (see
 ...
 * nine by separating *primitives* from *procedures*: `exec_command` can run git, so `git`
...
export function registerCoreTools(reg: SurfaceRegistrar): void {
  ...
  if (exposedCaps.read) {
    reg.register(
      'preview',
      {
        title: 'Preview a local website',
        description:
          'Start or reuse an app-managed local web preview for a folder containing index.html. ' +
          'Use this after creating or changing a static website so the user gets a compact Open preview link directly in MALACHI OVERDRIVE. ' +
          'The server runs inside the app; do not start python, npx, or another terminal server first.',
        inputSchema: z.object({
          path: pathArg.describe('Website folder inside an approved root. Relative paths resolve against this chat\'s folder.')
        }).strict(),
      ...
      async ({ path: requested }) =>
        guard('preview', async () => {
          if (!caps.read) {
            return fail('TOOL_DISABLED: preview needs file reading enabled in MALACHI OVERDRIVE.');
          }
          const workspace = currentWorkspace();
          const base = workspace?.virtual ?? (ctx.roots[0] ? `/${ctx.roots[0].name}` : null);
          const resolved = await resolveIn(ctx.roots, requested, { base });
          const stat = await fs.stat(resolved.real);
          if (!stat.isDirectory()) return fail('preview failed: path must be a folder containing index.html.');
          const index = nodePath.join(resolved.real, 'index.html');
          try {
            if (!(await fs.stat(index)).isFile()) return fail('preview failed: the folder does not contain index.html.');
          } catch {
          ...
          return {
            ...
            structuredContent: { url, path: resolved.virtual }
          };
        })
    );
  }
      ...
      async ({ paths, start_line, end_line, max_bytes }) =>
        guard('read', async () => {
          if (!caps.read && !caps.browse && !caps.metadata) {
            return fail(
              'TOOL_DISABLED: read is disabled by the current MALACHI OVERDRIVE permissions. Ask the user to enable reading in the app.'
            );
          }
    ...
    reg.register(
      ...
      async ({ path }) =>
        guard('view_image', async () => {
          if (!caps.read) {
            return fail(
              'TOOL_DISABLED: view_image is disabled by the current MALACHI OVERDRIVE permissions. Ask the user to enable reading in the app.'
            );
          }
      ...
      async ({ patch }) =>
        guard('apply_patch', async () => {
          if (!caps.create && !caps.edit && !caps.move && !caps.deleteFile) {
            return fail(
              'TOOL_DISABLED: apply_patch is disabled by the current MALACHI OVERDRIVE permissions. Ask the user to enable changing files in the app.'
            );
          }
      ...
      async (input) =>
        reg.guarded('command', 'write_stdin', async () => {
          ...
          if (execOwnershipDenied(input.session_id, asking)) {
            return fail(
              `write_stdin failed: session ${input.session_id} is not proven to belong to this durable MALACHI OVERDRIVE session. Start your own with exec_command or retry after the extension reconnects.`
            );
          }
      ...
      async ({ file, path: requestedPath }) =>
        guard('download_artifact', async () => {
          if (!caps.saveArtifact) {
            return fail(
              'TOOL_DISABLED: download_artifact is disabled by the current MALACHI OVERDRIVE permissions. Ask the user to enable saving ChatGPT files in the app.'
            );
          }
          try {
