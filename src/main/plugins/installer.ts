...
export async function runInstaller(command: string, args: string[], cwd: string): Promise<void> {
  await new Promise<void>((resolve, reject) => {
    ...
    child.once('error', () => {
      clearTimeout(timer);
      reject(new Error(`Required runtime ${path.basename(command)} is unavailable; install it and restart MALACHI OVERDRIVE`));
    });
  ...
  if (source.kind === 'npm') {
    ...
    if (process.platform === 'win32') {
      ...
      try {
        ...
        throw new Error('Install the standard Node.js distribution including npm, then restart MALACHI OVERDRIVE');
      }
...
async function findExecutable(name: string): Promise<string> {
  ...
  throw new Error(`Install ${name} and restart MALACHI OVERDRIVE`);
}
