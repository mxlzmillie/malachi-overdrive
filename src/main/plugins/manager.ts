...
export class PluginManager {
  ...
  private async startConnection(row: RecordEntry, signal: AbortSignal): Promise<void> {
    ...
    row.error = undefined;
    this.changed();
    const client = new Client({ name: 'MALACHI OVERDRIVE Plugins', version: '1.0.0' });
  ...
  async call(name: string, args: Record<string, unknown> = {},
    ...
    try {
      ...
      return errorResult(
        'PLUGIN_CALL_FAILED: The server failed or disconnected. The operation may have completed; inspect its state before retrying. MALACHI OVERDRIVE did not retry.',
      );
