...
export class PluginOAuth implements OAuthClientProvider {
  ...
  get clientMetadata() {
    return { client_name: 'MALACHI OVERDRIVE', redirect_uris: [this.redirectUrl], grant_types: ['authorization_code', 'refresh_token'], response_types: ['code'], token_endpoint_auth_method: 'none' };
  }
    ...
    try {
      ...
      server.on('request', (request, response) => {
        ...
        if (url.searchParams.has('error') || !url.searchParams.get('code')) {
          response.writeHead(400).end('Sign-in was not completed. Return to MALACHI OVERDRIVE.');
          fail(new Error('Plugin sign-in was not completed.')); return;
        }
        response.end('Authorization received. Return to MALACHI OVERDRIVE.'); server.close(); settle(url.searchParams);
      });
