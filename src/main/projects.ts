...
const projectSchema = z.object({ id: z.string().uuid(), name: z.string().min(1).max(160), path: z.string().min(1).max(32768), createdAt: z.number().finite().nonnegative(), kind: z.enum(['collection', 'work']).optional(), summary: z.string().max(600).optional(), brief: z.string().max(4000).optional(), category: z.string().max(80).optional() });
