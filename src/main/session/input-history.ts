...
export async function recordDeliveredInput(entry: Readonly<InputEntry>): Promise<boolean> {
  ...
  // Deleting a session also deletes the only valid destination for this projection. Treat the
  // history obligation as retired so every queue/status poll does not reopen the missing path
  // and warn forever. The transport receipt itself remains durable in the input ledger.
