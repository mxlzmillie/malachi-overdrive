 ...
 */
import { z } from 'zod';
import { browserInputModel, MAX_INPUT_TEXT_CHARS, INPUT_TEXT_LIMIT_MESSAGE, type InputImage } from '../../shared/input.js';
...
const planLineageSchema = z.object({
  id: z.string().uuid(),
  index: z.number().int().min(0).max(11),
  total: z.number().int().min(1).max(12)
}).strict().refine(plan => plan.index < plan.total, 'Plan stage index must be below its total');
const entrySchema = inputArgs.extend({
  ...
  plan: planLineageSchema.optional(),
  reroutedAt: z.number().optional(),
...
export async function sessionInputPolicy(sessionId: string, observedActivity?: boolean): Promise<{ queueAtFinish: boolean; canInject: boolean; browserAllowed: boolean }> {
  ...
  // The durable turn id is lifecycle history. The bridge's activity lease is the authority on
  // whether a current request can still collect tool input. Once that lease expires, treating
  // the old id as live strands messages forever because no later MCP call can collect them.
  // Tests and isolated callers without an activity observer retain the recorder-only fallback.
  const active = hasActivityObserver ? activity : !!session.activeTurnId;
  const canInject = !stopped && active;
  ...
  return { canInject, queueAtFinish: astra && canInject && getConfig().ui.finishTool === true,
    browserAllowed: !active && (!astra || (!activity && terminal)) };
}
...
async function expireQueued(current: InputEntry[]): Promise<InputEntry[]> {
  const next = await Promise.all(current.map(async (row): Promise<InputEntry> => {
    ...
    if (row.state === 'queued' && row.mode === 'auto' && !row.finishOwner &&
        ...
        Date.now() - (row.reroutedAt ?? Math.max(row.createdAt, row.dueAt)) >= 60_000)
    ...
    // After authorization allow 30s native acceptance, 120s canonical receipt and
    // 30s ACK transport. The claim stays spent throughout; this never grants a resend.
    if (row.state === 'browser' && Date.now() - (row.sendAuthorizedAt ?? row.offeredAt ?? row.createdAt) >= (row.sendAuthorizedAt === undefined ? (row.attachments?.length ? 720_000 : row.images?.length ? 120_000 : 60_000) : 180_000))
...
function append(current: InputEntry[], entry: InputEntry, stackDirect = false): InputEntry[] {
  ...
  const livePlans = new Set([...current, entry].filter(row => row.plan && !terminal(row)).map(row => row.plan!.id));
  const retainedPlanMember = (row: InputEntry): boolean => !!row.plan && livePlans.has(row.plan.id);
  const active = current.filter((row) => !terminal(row) || needsHistory(row) || pendingStages(row) || retainedPlanMember(row));
  ...
  const history = current.filter((row) => terminal(row) && !needsHistory(row) && !pendingStages(row) && !retainedPlanMember(row)).slice(-50);
...
export function enqueueInput(raw: InputArgs, finishOwner?: InputEntry['finishOwner']): Promise<InputEntry> {
  return serial(async () => {
    ...
    const entry: InputEntry = { ...input, ...(transportIntent ? { transportIntent } : {}), ...(requestedMode !== input.mode ? { requestedMode } : {}), ...(finishOwner ? { finishOwner } : {}),
      ...(input.stages !== undefined ? { plan: { id: input.id, index: 0, total: input.stages.length + 1 } } : {}),
...
export function listInputs(): Promise<InputEntry[]> {
  return serial(async () => {
    ...
    for (const entry of current) {
      // A legacy root owns its authored stage list exactly. Previously materialized
      // children have no durable parent identity, so they are never joined by timing
      // or text coincidence; roots awaiting materialization are upgraded atomically below.
      let migrated = entry.stages !== undefined && !entry.plan
        ? { ...entry, plan: { id: entry.id, index: 0, total: entry.stages.length + 1 } } satisfies InputEntry
        : entry;
      // A direct message elected tool delivery while a turn was live. If that turn ended
      // before any request collected the row, move the still-unclaimed message to the native
      // composer. A `tool` state is never changed, so an offered payload cannot be duplicated.
      if (deliveryHooks?.hasActivity && migrated.state === 'queued' && migrated.mode === 'auto' && migrated.transportIntent === 'tool' && migrated.sessionId) {
        const policy = await sessionInputPolicy(migrated.sessionId);
        if (!policy.canInject && policy.browserAllowed) migrated = { ...migrated, transportIntent: 'browser', reroutedAt: Date.now() };
      }
      if (migrated.purpose !== 'decision' && (migrated.state === 'sent' || (migrated.state === 'cancelled' && migrated.deliveredAt !== undefined)) && !migrated.sessionId && migrated.conversationId && !migrated.deliveredSessionId) {
        const session = await findSessionByConversation(migrated.conversationId, { requireUnique: true });
        next.push(session ? { ...migrated, deliveredSessionId: session.id } : migrated);
    ...
    for (const entry of [...next]) {
      ...
      const position = next.indexOf(entry); next[position] = { ...entry, plan, stagesApplied: true };
...
export function editQueuedInput(id: string, text: string, afterTurn?: boolean): Promise<boolean> {
  return serial(async () => {
    const value = inputTextSchema.parse(text);
    const current = await load();
