...
/**
 * Closes the exact recorder turn whose browser recovery window was exhausted.
 *
 * The browser can disappear after ChatGPT has rendered its final answer but before the
 * document publishes `turn_end`. Once the app's bounded silence recovery has also completed,
 * keeping that old turn open is no longer conservative: it makes every later desktop message
 * look injectable into a request that cannot call another tool. Serialize this verdict with
 * normal page observations so a real terminal event that arrived first always wins.
 */
export function endAbandonedTurn(
  conversationId: string,
  sessionId: string,
  expectedTurnId: string,
  outcome: Extract<TurnOutcome, 'interrupted' | 'stalled'>,
  detail: string
): Promise<boolean> {
  const prior = observationChains.get(conversationId) ?? Promise.resolve();
  const work = prior.then(async () => {
    const live = conversations.get(conversationId);
    if (!live || live.sessionId !== sessionId || live.turnId !== expectedTurnId ||
        live.turnStartedAt === null || live.knownTurnEnds.has(expectedTurnId)) return false;
    const current = await getSession(sessionId);
    if (!current || current.conversationId !== conversationId || current.activeTurnId !== expectedTurnId) return false;
    const time = Date.now();
    await appendEvent(sessionId, {
      time,
      source: 'app',
    ...
    live.knownTurnEnds.add(expectedTurnId);
    live.openTurns.delete(expectedTurnId);
