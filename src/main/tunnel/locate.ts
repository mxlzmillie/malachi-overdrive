...
export function locateBinary(name: BinaryName, hint?: string): string | null {
  ...
  if (hint && hint.trim() !== '') {
    ...
    // A configured path is an explicit choice, including which bytes may execute. If it is
    // missing or unusable, surface that error instead of silently launching a bundled or PATH
    // binary the user did not select.
