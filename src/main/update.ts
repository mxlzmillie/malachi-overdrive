/**
 * Whether a newer MALACHI OVERDRIVE has been published, and the file that becomes it.
 *
 ...
 * - **No manual replacement.** A staged update is applied during the ordinary quit sequence. The
 *   main process also restarts automatically once the laptop is idle and no browser turn, MCP
 *   request or retained terminal process is active. The Install button remains an optional "now"
 *   control, not a requirement for receiving updates.
 ...
 * update is on its way. macOS is out entirely **until releases are Developer-ID signed and
 * notarized with a stable identity**: silently replacing an ad-hoc build would hand Gatekeeper a
 * binary the user never chose to trust and can invalidate Accessibility/Screen Recording consent.
 *
...
const LATEST_RELEASE_API = RELEASE_REPOSITORY
  ? `https://api.github.com/repos/${RELEASE_REPOSITORY}/releases/latest`
  : null;

const CHECK_TIMEOUT_MS = 15_000;
const DOWNLOAD_TIMEOUT_MS = 10 * 60_000;
/** Give a freshly staged release time to settle before an automatic restart is even considered. */
export const AUTO_INSTALL_GRACE_MS = 5 * 60_000;
/** Require the whole laptop to be idle, not merely this window, before restarting itself. */
export const AUTO_INSTALL_IDLE_SECONDS = 120;
/**
...
export function stagedArtifact(
  ...
  if (platform === 'win32') return { name: `MALACHI-OVERDRIVE-Setup-${arch}.exe`, kind: 'installer', target: '' };
  if (platform === 'linux' && appImage) {
    return { name: `MALACHI-OVERDRIVE-Linux-${arch}.AppImage`, kind: 'appimage', target: appImage };
  }
  return null;
}
...
/**
 * Whether it is safe for the app to restart itself for a downloaded update right now.
 *
 * The updater owns the release bytes; the main process owns liveness. Keeping this decision pure
 * makes the safety rule explicit and testable without teaching the updater about browser turns,
 * terminal sessions or Electron windows. Every positive answer means all of those owners already
 * reported idle, the user has been away from the machine long enough, and the verified artifact
 * has sat ready through a grace window.
 */
export function automaticInstallSafe(input: {
  stage: UpdateStatus['stage'];
  readySince: number | null;
  now: number;
  systemIdleSeconds: number;
  inFlightRequests: number;
  generatingConversations: number;
  backgroundProcesses: number;
}): boolean {
  return (
    input.stage === 'ready' &&
    input.readySince !== null &&
    input.now - input.readySince >= AUTO_INSTALL_GRACE_MS &&
    input.systemIdleSeconds >= AUTO_INSTALL_IDLE_SECONDS &&
    input.inFlightRequests === 0 &&
    input.generatingConversations === 0 &&
    input.backgroundProcesses === 0
  );
}
...
export function startUpdateChecks(): void {
  if (!RELEASE_REPOSITORY) {
    logInfo('update: no trusted MALACHI OVERDRIVE release repository is baked into this build');
    return;
  }
...
async function latestVersion(): Promise<string> {
  if (!LATEST_RELEASE_API) throw new Error('this build has no trusted MALACHI OVERDRIVE release channel');
...
function assetUrl(version: string, name: string): string {
  if (!RELEASE_REPOSITORY) throw new Error('this build has no trusted MALACHI OVERDRIVE release channel');
...
export async function applyStagedUpdate(): Promise<void> {
  ...
  try {
    ...
    if (ready.kind === 'installer') {
      ...
      // Prefer one atomic rename from the verified staged file straight over the running
      // AppImage. Linux keeps the old inode alive for this process while the pathname moves to
      // the new release, so there is no reason to spend shutdown time copying hundreds of MB on
      // the normal same-filesystem case. External/mounted targets can be EXDEV; only then fall
      // back to a copy beside the target followed by the same atomic final rename.
      await chmod(ready.file, 0o755);
      try {
        await rename(ready.file, ready.target);
      } catch (error) {
