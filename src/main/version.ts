...
/**
 * Repository whose releases this exact build is allowed to trust for app/companion updates.
 * Release CI bakes `github.repository` into the main bundle. A local package deliberately has no
 * channel unless its builder opts in, so a branded checkout can never fall back to the public
 * upstream repository merely because package.json still records upstream project provenance.
 */
const bakedReleaseRepository =
  typeof __MALACHI_OVERDRIVE_RELEASE_REPOSITORY__ === 'string'
    ? __MALACHI_OVERDRIVE_RELEASE_REPOSITORY__.trim()
    : '';
export const RELEASE_REPOSITORY =
  bakedReleaseRepository ||
  (process.env.NODE_ENV === 'test'
    ? (process.env.MALACHI_OVERDRIVE_RELEASE_REPOSITORY ?? '').trim()
    : '');
...
export function extensionDownloadUrl(version = APP_VERSION): string {
  if (!RELEASE_REPOSITORY) throw new Error('MALACHI OVERDRIVE release channel is not configured');
  return `https://github.com/${RELEASE_REPOSITORY}/releases/download/v${encodeURIComponent(version)}/MALACHI-OVERDRIVE-Extension.zip`;
}
 ...
 * 7 — the app renamed itself to MALACHI OVERDRIVE, and the `app` field every bridge response
