import { showProjectFiles, showWorkLibrary } from './project-files.js';
...
import { MAX_INPUT_TEXT_CHARS, INPUT_TEXT_LIMIT_MESSAGE, type InputImage, type InputAttachment, type InputAutomation } from '../shared/input.js';
...
let commandCenter: ReturnType<typeof createCommandCenter> | null = null;
...
function paintSessions(): void {
  ...
  for (const id of projectIds) {
    ...
    heading.title = project ? `Browse files in ${project.name}` : 'Unavailable project';
    if (project) heading.addEventListener('click', event => {
      event.preventDefault();
      if (project.kind === 'collection') showWorkLibrary(projects, openOrganisedWork);
      else if (project.kind === 'work') openOrganisedWork(project);
      else showProjectFiles(project, () => selectNewChat(id));
    });
  ...
  const primary = projectSections.filter(section => projects.find(p => p.id === section.dataset.projectId)?.kind);
  const other = projectSections.filter(section => !projects.find(p => p.id === section.dataset.projectId)?.kind);
  ...
  $('workContext').hidden = !project;
  $('workContextLabel').textContent = project?.name ?? '';
  commandCenter?.refresh();
...
function paintPreparedPlan(): void {
  ...
  preview.replaceChildren(...plan.stages.map((stage, index) => {
    ...
    const field = document.createElement('textarea'); field.value = stage; field.hidden = true;
...
function toolBody(event: Extract<SessionEvent, { kind: 'tool_call' }>, context?: { id: string; current: () => boolean }): HTMLElement {
  ...
  if (call.tool === 'preview' && call.outcome === 'ok') {
    const match = call.result.text.match(/https?:\/\/(?:127\.0\.0\.1|localhost):\d+\/?[^\s)]*/i);
    const href = match?.[0] ? safeRenderedHref(match[0]) : null;
    if (href) {
      const link = el('a', 'preview-link');
      link.setAttribute('href', href);
      link.setAttribute('aria-label', 'Open local website preview');
      link.append(icon('i-out', 'ico'), el('span', '', 'Open preview'), el('small', '', new URL(href).host));
...
function eventRow(event: SessionEvent): HTMLElement {
  ...
  if (event.kind === 'tool_call' && event.call.tool === 'preview') row.classList.add('ev-preview');
...
function groupToolRows(rows: HTMLElement[], scope = selectedId, groups = toolGroups): HTMLElement[] {
  ...
  for (let i = 0; i < rows.length;) {
    if (!rows[i]!.matches('.ev-tool_call:not(.ev-preview), .ev-page_tool, .ev-agent_message')) { grouped.push(rows[i++]!); continue; }
    let end = i + 1;
    while (end < rows.length && rows[end]!.matches('.ev-tool_call:not(.ev-preview), .ev-page_tool, .ev-agent_message') && rows[end]!.dataset.activityBoundary === rows[i]!.dataset.activityBoundary) end++;
...
function stateLine(): { text: string; tone: '' | 'is-live' | 'is-bad' } {
  if (!deps.state()?.config.ui.developerMode) {
    ...
    const elapsed = `${seconds >= 60 ? `${Math.floor(seconds / 60)}m ` : ''}${seconds % 60}s`;
    if (!active) return { text: `Worked for ${elapsed}`, tone: '' };

    const starts = new Map<string, number>();
    const completed: number[] = [];
    for (const event of events) {
      if (event.kind === 'turn_start' && event.turnId) starts.set(event.turnId, event.time);
      if (event.kind === 'turn_end' && event.outcome === 'completed' && event.turnId && event.turnId !== turnId) {
        const start = starts.get(event.turnId);
        if (start !== undefined && event.time >= start) completed.push(event.time - start);
      }
    }
    const recent = completed.slice(-5).sort((a, b) => a - b);
    const learnedMs = recent.length ? recent[Math.floor(recent.length / 2)]! : 5 * 60_000;
    const elapsedMs = seconds * 1000;
    const forecastMs = Math.max(learnedMs, Math.ceil((elapsedMs + 60_000) / 60_000) * 60_000);
    return { text: `Working for ${elapsed} · estimated finish ${clockTime(startedAt + forecastMs)}`, tone: '' };
  }
...
async function refreshInputQueue(): Promise<void> {
  ...
  const stagedRoots: InputEntry[] = [];
  for (const entry of staged) {
    if (!entry.stages?.length) continue;
    const plan = entry.plan ?? { id: entry.id, index: 0, total: entry.stages.length + 1 };
    stagedRoots.push(entry.plan ? entry : { ...entry, plan });
    ...
    const id = `${entry.id}:stage:${index}`; projectedIds.add(id);
      queuedTasks.push({ ...entry, id, text, mode: 'finish', state: 'browser', stages: undefined,
        plan: { id: plan.id, index: index + 1, total: plan.total } });
    }
  }
  ...
  const planEntries = [...all.filter(entry => belongsToSelection(entry) && !!entry.plan), ...stagedRoots,
    ...queuedTasks.filter(entry => !!entry.plan && projectedIds.has(entry.id))];
  const planGroups = new Map<string, InputEntry[]>();
  for (const entry of planEntries) {
    const id = entry.plan?.id;
    if (!id) continue;
    const rows = planGroups.get(id) ?? [];
    if (!rows.some(row => row.id === entry.id)) rows.push(entry);
    planGroups.set(id, rows);
  }
  const selectedSummary = sessions.find(row => row.id === selectedId);
  const working = controlledSessionId === selectedId && controlledSelection === selectionGeneration
    ? controlledTurnId !== null : !!selectedSummary && sessionWorking(selectedSummary);
  const actualPlanEntries = [...planGroups.values()].flat().filter(entry => !projectedIds.has(entry.id));
  const byQueue = (a: InputEntry, b: InputEntry): number =>
    (a.queueOrder ?? a.dueAt) - (b.queueOrder ?? b.dueAt) || a.createdAt - b.createdAt || (a.plan?.index ?? 0) - (b.plan?.index ?? 0);
  const claimed = actualPlanEntries.filter(entry => ['browser', 'tool'].includes(entry.state)).sort(byQueue)[0];
  const newestSent = working ? actualPlanEntries.filter(entry => entry.state === 'sent')
    .sort((a, b) => (b.deliveredAt ?? b.createdAt) - (a.deliveredAt ?? a.createdAt))[0] : undefined;
  const nextQueued = actualPlanEntries.filter(entry => entry.state === 'queued').sort(byQueue)[0];
  const activePlanInputId = claimed?.id ?? newestSent?.id ?? nextQueued?.id ?? null;
  const activePlans = [...planGroups.entries()].filter(([, entries]) => entries.some(entry =>
    ['queued', 'browser', 'tool'].includes(entry.state) || entry.id === activePlanInputId));
  const activePlanIds = new Set(activePlans.map(([id]) => id));
  const taskList = $('finishQueue');
  const ordinaryTasks = queuedTasks.filter(entry => !entry.plan || !activePlanIds.has(entry.plan.id));
  taskList.hidden = activePlans.length === 0 && ordinaryTasks.length === 0;
  taskList.classList.toggle('has-plan', activePlans.length > 0);
  const oldCards = new Map([...taskList.querySelectorAll<HTMLElement>('[data-input-id]')].map(node => [node.dataset.inputId, node]));
  const dragging = !!taskList.querySelector('.is-dragging');
  const taskCard = (entry: InputEntry, planState?: 'complete' | 'active' | 'future' | 'failed'): HTMLElement => {
    ...
    if (planState && entry.plan) {
      card.classList.add('plan-step', `is-${planState}`);
      card.setAttribute('aria-label', `Plan step ${entry.plan.index + 1} of ${entry.plan.total} · ${planState}`);
    } else if (projectedIds.has(entry.id)) card.setAttribute('aria-label', 'Plan stage · waiting for the first message to be sent');
    ...
    if (planState && entry.plan) {
      const marker = el('span', 'plan-step-marker', planState === 'complete' ? '' : String(entry.plan.index + 1));
      if (planState === 'complete') marker.append(icon('i-check'));
      card.append(marker, label);
    } else card.append(icon('i-clock'), label);
    if (entry.state === 'queued') {
      ...
      edit.onclick = () => {
        const field = document.createElement('textarea'); field.value = entry.text; field.setAttribute('aria-label', 'Queued task');
        const validation = el('p', 'queue-edit-validation'); validation.id = `queue-error-${entry.id}`; validation.setAttribute('role', 'status');
        field.setAttribute('aria-describedby', validation.id);
        const validate = () => {
          const value = field.value.trim();
          const error = !value ? 'Enter a task before saving.' : value.length > MAX_INPUT_TEXT_CHARS ? INPUT_TEXT_LIMIT_MESSAGE : '';
          validation.textContent = error; field.setAttribute('aria-invalid', String(!!error));
          return !error;
        };
        ...
        save.onclick = async () => {
          if (save.disabled || !validate()) return;
        ...
        const back = el('button', 'btn', 'Cancel edit') as HTMLButtonElement; back.type = 'button';
        back.onclick = () => { if (save.disabled) return; card.classList.remove('is-editing'); card.replaceChildren(...contents); edit.focus(); };
        card.classList.add('is-editing'); card.replaceChildren(field, save, back, validation); field.focus();
      };
  ...
  for (const [planId, entries] of activePlans.sort(([, a], [, b]) => Math.min(...a.map(row => row.createdAt)) - Math.min(...b.map(row => row.createdAt)))) {
    const orderedPlan = [...entries].sort((a, b) => (a.plan?.index ?? 0) - (b.plan?.index ?? 0));
    const total = orderedPlan[0]?.plan?.total ?? orderedPlan.length;
    const completed = orderedPlan.filter(entry => entry.state === 'sent' && entry.id !== activePlanInputId).length;
    const header = el('div', 'plan-header'); header.dataset.planId = planId;
    const title = el('span', 'plan-header-title'); title.append(icon('i-steps'), el('strong', '', 'Plan'));
    header.append(title, el('span', 'plan-count', `${completed} / ${total}`));
    planNodes.push(header);
    for (const entry of orderedPlan) {
      const state = entry.id === activePlanInputId ? 'active' : entry.state === 'sent' ? 'complete'
        : ['failed', 'cancelled'].includes(entry.state) ? 'failed' : 'future';
      planNodes.push(taskCard(entry, state));
    }
  }
  reconcileChildren(taskList, [...planNodes, ...ordinaryTasks.map(entry => taskCard(entry))]);
...
async function sendComposer(delivery?: 'finish', plan?: string[]): Promise<boolean | void> {
  ...
  if (text.length > MAX_INPUT_TEXT_CHARS) { toast(INPUT_TEXT_LIMIT_MESSAGE); return false; }
  ...
  paintDeliveryControls();
  let accepted = false;
  const restoreUnconfirmedDraft = () => {
    if (selectedId === sessionId && selectionGeneration === generation && !input.value) {
      input.value = authoredDraft;
      input.style.height = 'auto'; input.style.height = `${Math.min(220, input.scrollHeight)}px`;
  ...
  try {
    ...
    // A rejected IPC promise is not proof that the backend never accepted the message.
    // Preserve the draft, never retry automatically, and never overwrite a newer edit.
    if (!accepted && !cancelledStarts.has(id)) restoreUnconfirmedDraft();
    toast(accepted ? 'The message was queued, but a follow-up control could not be confirmed.'
      : `Could not confirm delivery. Your draft is kept; check the queue before resending. ${error instanceof Error ? error.message : ''}`);
    return accepted;
...
function openOrganisedWork(project: LocalProject): void {
  selectNewChat(project.id, true);
  const input = $<HTMLTextAreaElement>('chatInput');
  // Returning to a project resumes its unsent work, including staged attachments.
  if (input.value || imageDrafts.get(draftKey())?.length) return;
  input.value = projectContinuation(project);
  input.dispatchEvent(new Event('input', { bubbles: true }));
  input.focus(); input.setSelectionRange(input.value.length, input.value.length);
}

function selectNewChat(projectId: string | null = null, preserveDraft = false): void {
  ...
  if (!preserveDraft) { inputDrafts.delete(draftKey()); imageDrafts.delete(draftKey()); }
...
function selectedWork(): LocalProject | undefined {
  const projectId = selectedId ? sessions.find(row => row.id === selectedId)?.projectId ?? selectedProjectId : selectedProjectId;
...
function paintWorkbench(): void {
  ...
  const chat = (row: SessionSummary): WorkbenchChat => ({
    id: row.id, title: row.title || 'Untitled chat', project: projects.find(project => project.id === row.projectId)?.name ?? '',
    active: sessionWorking(row), draft: !!inputDrafts.get(row.id) || !!imageDrafts.get(row.id)?.length
  });
  const ordinary = sessions.filter(row => row.conversationId && row.origin?.kind !== 'worker');
  const cards = projects.filter(row => row.kind === 'work').map(project => {
    const latest = ordinary.find(row => row.projectId === project.id);
    return { id: project.id, name: project.name, category: project.category ?? '', summary: project.summary ?? '',
      draft: !!inputDrafts.get(`project:${project.id}`) || !!imageDrafts.get(`project:${project.id}`)?.length,
      latest: latest ? chat(latest) : null, updatedAt: latest?.updatedAt ?? project.createdAt };
  }).sort((a, b) => Number(b.draft) - Number(a.draft) || b.updatedAt - a.updatedAt);
  workbench.update({ projectName: selectedWork()?.name ?? null, existingChat: selectedId !== null,
    projects: cards.map(({ updatedAt: _updatedAt, ...card }) => card),
    recent: ordinary.filter(row => !selectedProjectId || row.projectId === selectedProjectId).slice(0, 3).map(chat) });
}

function prepareRecipe(recipe: typeof WORK_RECIPES[number]): void {
  const input = $<HTMLTextAreaElement>('chatInput');
  const value = appendWorkRecipe(input.value, recipe);
  deps.navigate?.('chat');
  input.value = value;
  input.dispatchEvent(new Event('input', { bubbles: true }));
  rememberDraft(); input.focus(); input.setSelectionRange(value.length, value.length);
  toast('Task brief added to your draft. Review it before sending.');
}

function commandSnapshot(): CommandSnapshot {
  const state = deps.state(), project = selectedWork();
  const navigate = (name: string) => { deps.navigate?.(name); };
  const entries: WorkCommand[] = [
    ...projects.filter(row => row.kind === 'work').map(row => ({
      id: `project:${row.id}`, label: row.name, detail: row.summary ?? 'Continue this project with its existing context.',
      group: 'projects' as const, icon: 'i-folder', keywords: row.category, badge: 'project',
      run: () => { navigate('chat'); openOrganisedWork(row); }
    })),
    ...sessions.filter(row => row.conversationId).map(row => ({
      id: `chat:${row.id}`, label: row.title || 'Untitled chat',
      detail: [projects.find(work => work.id === row.projectId)?.name ?? 'Conversation', ...sessionBadges(row).map(badge => badge.text), ago(row.updatedAt)].join(' · '),
      group: 'chats' as const, icon: 'i-chat', active: sessionWorking(row),
      issues: blockedChats.has(row.conversationId!) || row.errors > 0 || row.toolInternalErrors > 0 || row.processExitNonzero > 0 || row.toolRejected > 0,
      badge: blockedChats.has(row.conversationId!) ? 'blocked' : sessionWorking(row) ? 'active' : 'chat',
      run: () => { navigate('chat'); pendingNewInput = null; if (selectedId !== row.id) selectSession(row.id); $<HTMLTextAreaElement>('chatInput').focus(); }
    })),
    ...WORK_RECIPES.map(recipe => ({
      id: `recipe:${recipe.id}`, label: `Prepare: ${recipe.label}`, detail: recipe.detail,
      group: 'actions' as const, icon: recipe.icon, keywords: 'brief workflow task', badge: 'draft only', run: () => prepareRecipe(recipe)
    })),
    { id: 'action:files', label: 'Browse current project files', detail: project?.name ?? 'Select a project first.', group: 'actions', icon: 'i-folder', disabled: project ? undefined : 'Select a project first.', run: () => { const work = selectedWork(); if (work) showProjectFiles(work, () => { navigate('chat'); openOrganisedWork(work); }); } },
    { id: 'action:library', label: 'Open my work library', detail: 'Your curated projects, grouped by purpose.', group: 'actions', icon: 'i-home', run: () => showWorkLibrary(projects, row => { navigate('chat'); openOrganisedWork(row); }) },
    { id: 'action:focus', label: 'Focus the composer', detail: 'Return to your current draft without sending it.', group: 'actions', icon: 'i-pencil', run: () => { navigate('chat'); $<HTMLTextAreaElement>('chatInput').focus(); } },
    { id: 'action:activity', label: 'Open activity and diagnostics', detail: 'Inspect actual tool activity and recorded problems.', group: 'actions', icon: 'i-pulse', run: () => navigate('activity') },
    { id: 'action:settings', label: 'Open workspace settings', detail: 'Review existing permissions and approved folders.', group: 'actions', icon: 'i-gear', run: () => navigate('home') },
    { id: 'action:usage', label: 'Open usage', detail: 'Review the usage information available to this app.', group: 'actions', icon: 'i-pulse', run: () => navigate('usage') }
  ];
  return {
    entries, context: project ? `Working in ${project.name} · project drafts stay with their project` : 'Jump between your work without sending a message.',
    browser: state?.bridge.present ?? null, readOnly: state?.config.readOnly ?? null,
    loadedRecordings: sessions.length, totalRecordings: Math.max(sessionTotal, sessions.length),
    loadingRecordings: sessionPageLoading,
    loadMoreRecordings: sessionPageCursor && sessions.length < sessionTotal ? loadMoreSessions : undefined
  };
}

export function initChat(next: Deps): void {
  deps = next;
  commandCenter = createCommandCenter({ trigger: $<HTMLButtonElement>('overdriveCommand'), snapshot: commandSnapshot, error: toast });
  workbench = createWorkbench($('workbench'), {
    project: id => { const project = projects.find(row => row.id === id && row.kind === 'work'); if (project) openOrganisedWork(project); },
    chat: id => { if (sessions.some(row => row.id === id && row.conversationId)) { pendingNewInput = null; selectSession(id); $<HTMLTextAreaElement>('chatInput').focus(); } },
    recipe: id => { const recipe = WORK_RECIPES.find(row => row.id === id); if (recipe) { try { prepareRecipe(recipe); } catch (error) { toast(error instanceof Error ? error.message : 'Could not prepare the task brief.'); } } },
