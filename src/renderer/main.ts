...
function buildGroups(): void {
  ...
  // permissions — `session` and `agents` are two of the ten tools ChatGPT can discover —
...
function paintDesktopAccess(next: AppState): void {
  ...
  $('desktopAccessDetail').textContent =
    `${missing.join(' · ')}. These are live verdicts from the native backend executing inside MALACHI OVERDRIVE. ` +
  ...
  if (update.latest) {
    ...
    lines.push(
      update.stage === 'checking'
        ...
        : update.stage === 'ready'
        ? `MALACHI OVERDRIVE ${update.latest} is downloaded and ready. It will restart automatically when your work is idle.`
        : update.stage === 'downloading'
          ? `MALACHI OVERDRIVE ${update.latest} is downloading. Keep working; you can install it when it lands.`
          : update.stage === 'failed'
            ? `MALACHI OVERDRIVE ${update.latest} could not be downloaded: ${update.error ?? 'the download stopped'}.`
            : `MALACHI OVERDRIVE ${update.latest} is out. This installation has to be updated by hand.`
    );
    ...
    lines.push(`Up to date! MALACHI OVERDRIVE ${update.current}${extension}`);
    tone = 'ok';
  }
  if (stale) {
    lines.push(
...
function paintUpdate(next: AppState): void {
  ...
  // `ready` is the only state with a verified artifact on disk. Restart-now remains an optional
  // override; users do not need it because the main process also installs automatically when idle.
...
function facts(next: AppState): HTMLElement[] {
  ...
  if (isRunning(status.state)) {
    ...
    if (status.localUrl) {
      let address = 'private loopback';
      try { address = new URL(status.localUrl).host; } catch { /* Keep the private fallback. */ }
      rows.push(['Local bridge', address]);
    }
  } else {
...
function installUpdate(): void {
  toast('Installing the update. MALACHI OVERDRIVE closes and starts again as the new version.');
...
initChat({ save: () => save(), state: () => state, navigate: showTab });
