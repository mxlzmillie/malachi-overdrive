...
function harness(initial: Tab[], cached?: number) {
  ...
  const windowCreate = vi.fn(async (options: { url: string }) => {
    const id = next++; windows.set(id, { id, state: 'minimized', focused: false });
    return { id, state: 'minimized', focused: false, tabs: [await create({ url: options.url, windowId: id })] };
  });
  ...
  const api = vm.runInNewContext(`${code}\n({ createChatTab, reconcileBackgroundWindow, revealWorkerChat })`, {
    URL, URLSearchParams, Promise, commandMarkerId: (value: unknown) => typeof value === 'string' ? value : null,
    ...
    chrome: {
      ...
      tabs: { query: async () => [...tabs.values()].map(tab => ({ ...tab })), create, move, get, update },
  ...
  }) as { createChatTab(url: string, background: boolean, active?: boolean): Promise<Tab>; reconcileBackgroundWindow(policy: object): Promise<boolean>; revealWorkerChat(request: object): Promise<void> };
  return { ...api, tabs, windows, stored, create, move, get, update, windowCreate, windowUpdate, call };
}
...
it('serializes explicit reveal against a concurrent background create', async () => {
  ...
  app.windowUpdate.mockImplementationOnce(async (id: number, patch: { state?: string; focused?: boolean }) => {
    await held;
    const window = app.windows.get(id)!; Object.assign(window, patch); return { ...window };
  });
  const reveal = app.revealWorkerChat({ id: 'show-worker', conversationId: 'worker' });
  await vi.waitFor(() => expect(app.windowUpdate).toHaveBeenCalledWith(9, { state: 'normal', focused: true }));
