        ...
        events: [
          {
            kind: 'user_message',
            time: Date.now(),
            text: 'Continue the previous MALACHI OVERDRIVE session. Read the handoff below.',
...
describe('unattributed activity recovery', () => {
    ...
    await events(PRIME, [
      {
        kind: 'user_message',
        time: Date.now(),
        text: '[[CLF-RESUME:O8THi8gMTC6LvH9GLclDIQ]]\n\nContinuing a MALACHI OVERDRIVE session that was compacted.',
  ...
  for (const model of ['GPT-6 Pro', 'GPT-5.6 Pro', 'gpt-5-6-pro', 'gpt-5-5-pro']) it(`keeps ${model} silent until ten minutes without inventing a Goal final`, async () => {
    ...
    try {
      ...
      expect(await getSession(live.body.sessionId)).toMatchObject({ activeTurnId: null, lastTurnOutcome: 'stalled' });
...
describe('the goal loop over the bridge', () => {
  ...
  it('keeps saved per-chat Goal state dormant while session recording is off', async () => {
    await pair();
    const chat = 'cafe0080-0000-4000-8000-000000000080';
    await saveConfig({
      ...defaultConfig(),
      sessions: { ...defaultConfig().sessions, record: true },
      multiAgent: { ...defaultConfig().multiAgent, enabled: true },
      // Start from the exact state that exposes the bug: the app-wide switch is already Off,
      // while one chat has its own Goal/Loop answer and objective.
      goal: { ...defaultConfig().goal, backend: 'api', enabled: false }
    });
    const recorded = await request('POST', '/events', {
      body: {
        conversationId: chat,
        events: [{ kind: 'user_message', time: Date.now(), text: 'Finish the migration', messageId: 'm-recording-gate' }]
      }
    });
    expect(recorded.status).toBe(200);
    expect((await request('POST', '/goal/objective', {
      body: { conversationId: chat, text: 'Finish the migration and verify it', mode: 'loop' }
    })).status).toBe(200);
    await acceptGoalReplyNow({
      conversationId: chat,
      sessionId: recorded.body.sessionId,
      replyId: 'recording-gate-final',
      turnId: 'recording-gate-turn',
      eventSeq: 1,
      blocked: false
    });
    expect(goalPendingReplyFor(chat)?.replyId).toBe('recording-gate-final');

    // Multi-agent mode deliberately keeps the shared browser bridge alive after recording is
    // disabled. Stored Goal policy must survive this privacy change, but it must become inert.
    await saveConfig({
      ...getConfig(),
      sessions: { ...getConfig().sessions, record: false },
      multiAgent: { ...getConfig().multiAgent, enabled: true }
    });
    expect(getConfig().sessions.record).toBe(false);
    expect(goalObjectiveFor(chat)).toBe('Finish the migration and verify it');
    expect(goalSwitchFor(chat)).toMatchObject({ enabled: true, mode: 'loop', own: true });

    const dormantSwitch = await request('POST', '/settings', {
      body: { conversationId: chat, loop: true }
    });
    expect(dormantSwitch.body.goal).toMatchObject({ enabled: false, own: true, mode: 'loop' });
    const dormantObjective = await request('POST', '/goal/objective', {
      body: { conversationId: chat, text: 'Finish the migration and run the final checks', mode: 'loop' }
    });
    expect(dormantObjective.body).toMatchObject({
      objective: 'Finish the migration and run the final checks', enabled: false, own: true, mode: 'loop'
    });
    expect(goalObjectiveFor(chat)).toBe('Finish the migration and run the final checks');
    expect(goalSwitchFor(chat)).toMatchObject({ enabled: true, mode: 'loop', own: true });

    const feed = await request('GET', `/activity?conversationId=${chat}`);
    expect(feed.body.goal).toMatchObject({ enabled: false, objective: '', pending: null, draft: null });
    expect(await sessionControlsFor(recorded.body.sessionId)).toMatchObject({
      objective: 'Finish the migration and run the final checks', automation: 'off', goalDraft: null
    });
    ...
    globalThis.fetch = (async () => {
      providerCalls++;
      return Response.json({ choices: [{ message: { content: JSON.stringify({ action: 'continue', reply: 'keep going' }) } }] });
    }) as never;
    try {
      const continued = await request('POST', '/goal/draft', {
        body: { conversationId: chat, turnId: 'recording-gate-turn' }
      });
      expect(continued.status).toBe(409);
      expect(continued.body.error).toBe('goal_disabled');

      const opened = await request('POST', '/goal/open', {
        body: { text: 'Finish the migration and verify it', mode: 'loop' }
      });
      expect(opened.status).toBe(409);
      expect(opened.body.error).toBe('goal_disabled');
  ...
  it('revokes an already queued Goal recovery when session recording turns off', async () => {
    vi.useFakeTimers();
    try {
      await pair();
      const chat = 'cafe0082-0000-4000-8000-000000000082';
      await request('POST', '/events', { body: { conversationId: chat, events: [
        { kind: 'user_message', time: Date.now(), text: 'keep going', messageId: 'm-recording-recovery' },
        { kind: 'assistant_message', time: Date.now(), messageId: 'a-recording-recovery', text: 'Part one is done.',
...
describe('app requests to stop one exact active turn', () => {
  it('does not present a persisted unfinished turn as live after recorder restart', async () => {
    ...
    await pair();
    expect((await request('GET', '/activity?conversationId=e5555555-aaaa-4bbb-8ccc-111111111111')).body.activeTurnId).toBeNull();
    expect(await getSession(sessionId)).toMatchObject({ activeTurnId: null, lastTurnOutcome: 'interrupted' });
  });
