...
it('keeps Opera Air as a first-class saved browser family', async () => {
  const config = defaultConfig();
  await saveConfig({ ...config, ui: { ...config.ui, chatBrowser: 'opera-air' } });
  expect((await loadConfig()).ui.chatBrowser).toBe('opera-air');
  expect(preferredBrowserCandidates('darwin', {}, '/Users/example', 'opera-air')).toEqual([
    '/Applications/Opera Air.app/Contents/MacOS/Opera Air',
    '/Users/example/Applications/Opera Air.app/Contents/MacOS/Opera Air'
  ]);
});
