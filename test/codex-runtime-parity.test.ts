...
describe('Codex unified exec runtime parity', () => {
  ...
  it.runIf(process.platform !== 'win32')('terminating a POSIX PTY session kills descendants in its process group', async () => {
    ...
    const initial = instance.execCommand({
      ...
      processId,
      // Keep the initial call active while the separately scheduled grandchild publishes
      // readiness. A 250 ms yield races machine load instead of exercising termination.
      yieldTimeMs: 30_000,
      maxOutputTokens: undefined,
      truncationPolicy,
      cwd: root,
