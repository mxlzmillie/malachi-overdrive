...
describe('desktop helper retirement ordering', () => {
  ...
  it('does not release the queue or spawn a replacement until a timed-out helper is dead', async () => {
    ...
    // Window metadata keeps its short steady-state deadline, while a cold native Windows ARM
    // helper gets a longer one-time compilation allowance before stdin is serviceable.
    await vi.advanceTimersByTimeAsync(
      helperTimeoutMs({ op: 'windows' }) + helperStartupGraceMs()
    );
    await Promise.resolve();
