...
it.each(['empty', 'draft', 'navigated', 'moved-to-personal', 'rejected', 'transport'])('terminal worker failure retires only the exact empty document (%s)', async mode => {
  const h = await worker([]);
  const source = { tab: 8, documentId: 'failed-worker', navigationEpoch: 1 };
  h.tabs.push({ id: 8, windowId: 80, url: `https://chatgpt.com/?clf=${firstId}` });
  h.saved.chatBackgroundWindow = 80; h.saved.chatBackgroundTabs = [8];
  ...
  h.sendMessage.mockImplementation(async (_tabId, message): Promise<any> => {
    if (message.type === 'clf-tab-close-check') {
      ...
      if (mode === 'moved-to-personal') h.tabs[0]!.windowId = 3;
...
type Tab = { id: number; url?: string; pendingUrl?: string; windowId?: number; active?: boolean; status?: string; discarded?: boolean };
async function worker(inputs: Array<{ id: string; conversationId: string | null; supersededConversationId?: string }>, modelCatalogRequest?: { nonce: string; expiresAt: number }, priorLocal: Record<string, unknown> = {}) {
  ...
  const windows = {
    ...
    update: vi.fn(async (id: number, patch: Partial<typeof ownedWindow>) => {
      ...
      return id === 80 ? { ...ownedWindow } : { id, ...patch };
    })
  };
...
describe('one browser maintenance flight per desktop outbox publication', () => {
  ...
  it('spends input opening authority before creation and permits a new explicit operation', async () => {
    ...
    await h.maintain();
    expect(h.fetch.mock.calls.some(([url]) => new URL(url).pathname === '/input/background-failed')).toBe(true); await h.maintain();
    expect(h.create).toHaveBeenCalledTimes(1);
  ...
  it('wakes an exact discarded conversation tab once before offering its input', async () => {
    const h = await worker([{ id: firstId, conversationId: secondId }]);
    h.tabs.push({ id: 7, url: `https://chatgpt.com/c/${secondId}`, discarded: true });
    await h.maintain();
    expect(h.reload).toHaveBeenCalledTimes(1);
    expect(h.sendMessage.mock.calls.some(([, message]) => message.type === 'clf-desktop-input')).toBe(false);
    await h.maintain();
    expect(h.reload).toHaveBeenCalledTimes(1);
    h.tabs[0]!.discarded = false;
    await h.maintain();
    expect(h.sendMessage).toHaveBeenCalledWith(7, expect.objectContaining({ type: 'clf-desktop-input', id: firstId, conversationId: secondId }));
    expect(h.create).not.toHaveBeenCalled();
  });
  it('reloads an exact input tab once when its content-script receiver is missing', async () => {
    const h = await worker([{ id: firstId, conversationId: secondId }]);
    h.tabs.push({ id: 7, url: `https://chatgpt.com/c/${secondId}` });
    h.sendMessage.mockRejectedValue(new Error('Could not establish connection. Receiving end does not exist.'));
    await h.maintain();
  ...
  it('closes an explicitly retired temporary planner without requiring another work tab', async () => {
    ...
    await h.maintain(); await h.settleRetirements();
  ...
  it.each([true, false])('places workers in isolation independent of background preference=%s', async background => {
    const h = await worker([]);
    let offered = true;
    h.fetch.mockImplementation(async (input) => ({ ok: true, status: 200, json: async () => {
      ...
      offered = false;
      return { ok: true, placement, inputs: [], background };
    } }));
    await h.maintain();
    expect(h.windows.create).toHaveBeenCalledWith(expect.objectContaining({ focused: false, state: 'minimized' }));
    expect(h.windows.update).not.toHaveBeenCalled();
...
it('a slow receipt in one chat cannot block another input or repeat its outstanding offer', async () => {
  const h = await worker([{ id: firstId, conversationId: null }, { id: secondId, conversationId: null }]);
  h.tabs.push({ id: 7, url: `https://chatgpt.com/?cos-input=${firstId}` }, { id: 8, url: `https://chatgpt.com/?cos-input=${secondId}` });
  let release!: () => void;
  const receipt = new Promise<void>(resolve => { release = resolve; });
  h.sendMessage.mockImplementation(async (tab, message) => {
    if (tab === 7 && message.type === 'clf-desktop-input') await receipt;
    return { ok: true };
  });
  const pass = h.maintain();
  try {
    await vi.waitFor(() => expect(h.sendMessage).toHaveBeenCalledWith(8, expect.objectContaining({ type: 'clf-desktop-input' })), { timeout: 500 });
    await pass; await h.maintain();
    expect(h.sendMessage.mock.calls.filter(([tab, message]) => tab === 7 && message.type === 'clf-desktop-input')).toHaveLength(1);
    expect(h.create).not.toHaveBeenCalled();
  } finally { release(); await pass; }
});

it('a silent protected recorder cannot hold unrelated desktop sends behind its health check', async () => {
  const h = await worker([{ id: firstId, conversationId: null }]);
  h.tabs.push({ id: 7, url: `https://chatgpt.com/c/${secondId}` },
    { id: 8, url: `https://chatgpt.com/?cos-input=${firstId}` });
  h.fetch.mockImplementation(async input => ({ ok: true, status: 200, json: async () =>
    new URL(input).pathname === '/hello'
      ? { app: 'chat-on-steroids', bridge: BRIDGE_PROTOCOL, compatible: true, paired: true }
      : { ok: true, inputs: [{ id: firstId, conversationId: null }], nonDiscardableConversations: [secondId] }
  }));
  let release!: () => void;
  const held = new Promise<void>(resolve => { release = resolve; });
  h.sendMessage.mockImplementation(async (_tab, message) => {
    if (message.type === 'clf-recorder-ping') await held;
    return { ok: true, recorderVersion: 11 };
  });
  const pass = h.maintain();
  try {
    await vi.waitFor(() => expect(h.sendMessage).toHaveBeenCalledWith(8,
      expect.objectContaining({ type: 'clf-desktop-input', id: firstId })), { timeout: 500 });
    await pass;
    await h.maintain();
    expect(h.sendMessage.mock.calls.filter(([, m]) => m.type === 'clf-recorder-ping')).toHaveLength(1);
    expect(h.create).not.toHaveBeenCalled();
...
it('a silent retired tab cannot hold later input maintenance or close after a newer protection policy', async () => {
  const h = await worker([]);
  h.tabs.push({ id: 7, url: `https://chatgpt.com/c/${firstId}` },
    { id: 8, url: `https://chatgpt.com/c/${secondId}` });
  await h.authorizeDocument({ tab: { id: 7 }, documentId: 'retired-document', frameId: 0, url: h.tabs[0]!.url }, { navigationEpoch: 1 });
  let policy: Record<string, unknown> = { inputs: [], managedConversations: [firstId], retiredConversations: [firstId] };
  h.fetch.mockImplementation(async input => ({ ok: true, status: 200, json: async () =>
    new URL(input).pathname === '/hello'
      ? { app: 'chat-on-steroids', bridge: BRIDGE_PROTOCOL, compatible: true, paired: true }
      : { ok: true, ...policy }
  ...
  let release!: () => void;
  const held = new Promise<void>(resolve => { release = resolve; });
  h.sendMessage.mockImplementation(async (tab, message): Promise<any> => {
    if (tab === 7 && message.type === 'clf-tab-close-check') {
      await held;
      return { safe: true, conversationId: firstId, navigationEpoch: 1 };
    }
    return { ok: true, recorderVersion: 11 };
  });
  const firstPass = h.maintain();
  let nextPass: Promise<void> | undefined;
  try {
    await vi.waitFor(() => expect(h.sendMessage).toHaveBeenCalledWith(7,
      expect.objectContaining({ type: 'clf-tab-close-check' }), { documentId: 'retired-document' }));
    policy = { ...policy, inputs: [{ id: secondId, conversationId: secondId }], nonDiscardableConversations: [firstId] };
    nextPass = h.maintain(true);
    await vi.waitFor(() => expect(h.sendMessage).toHaveBeenCalledWith(8,
      expect.objectContaining({ type: 'clf-desktop-input', id: secondId })), { timeout: 500 });
    await nextPass;
    await h.maintain(true);
    expect(h.sendMessage.mock.calls.filter(([, message]) => message.type === 'clf-tab-close-check')).toHaveLength(1);
    expect(h.create).not.toHaveBeenCalled();
  } finally {
    release();
    await firstPass; await nextPass; await h.settleRetirements();
  }
  expect(h.remove).not.toHaveBeenCalled();
});


it('a silent temporary planner cannot block other sends or retire after its policy is withdrawn', async () => {
  const inputs: Array<any> = [{ id: firstId, conversationId: null, owner: '7:planner:1', lifetime: 'temporary-planner', close: true, retire: true }];
  const h = await worker(inputs);
  h.tabs.push({ id: 7, url: `https://chatgpt.com/?cos-input=${firstId}` },
    { id: 8, url: `https://chatgpt.com/c/${secondId}` });
  await h.authorizeDocument({ tab: { id: 7 }, documentId: 'planner', frameId: 0, url: h.tabs[0]!.url }, { navigationEpoch: 1 });
  let release!: () => void;
  const held = new Promise<void>(resolve => { release = resolve; });
  h.sendMessage.mockImplementation(async (tab, message): Promise<any> => {
    if (tab === 7 && message.type === 'clf-close-temporary-planner') {
      await held;
      return { safe: true };
    }
    return { ok: true };
  ...
  try {
    await vi.waitFor(() => expect(h.sendMessage).toHaveBeenCalledWith(7,
      expect.objectContaining({ type: 'clf-close-temporary-planner' }), { documentId: 'planner' }));
    inputs.push({ id: secondId, conversationId: secondId });
    nextPass = h.maintain(true);
    await vi.waitFor(() => expect(h.sendMessage).toHaveBeenCalledWith(8,
      expect.objectContaining({ type: 'clf-desktop-input', id: secondId })), { timeout: 500 });
    await nextPass;
    expect(h.sendMessage.mock.calls.filter(([, message]) => message.type === 'clf-close-temporary-planner')).toHaveLength(1);
    inputs.splice(0, 1);
    await h.maintain(true);
    expect(h.create).not.toHaveBeenCalled();
  } finally {
...
it.each(['keeper-closed', 'candidate-selected', 'moved-to-personal'])('duplicate retirement rechecks native tabs after its draft proof: %s', async scenario => {
  const h = await worker([]);
  h.tabs.push({ id: 7, url: `https://chatgpt.com/c/${firstId}`, active: true },
    { id: 8, url: `https://chatgpt.com/c/${firstId}` });
  await h.maintain();
  await h.authorizeDocument({ tab: { id: 8 }, documentId: 'duplicate', frameId: 0, url: h.tabs[1]!.url }, { navigationEpoch: 1 });
  let release!: () => void;
  const held = new Promise<void>(resolve => { release = resolve; });
  h.sendMessage.mockImplementation(async (_tab, message): Promise<any> => {
    if (message.type === 'clf-tab-close-check') await held;
    return { safe: true, conversationId: firstId, navigationEpoch: 1 };
  });
  const retirement = h.pruneManagedTabs([...h.tabs], { managedConversations: [firstId] }, new Set(), new Set());
  try {
    await vi.waitFor(() => expect(h.sendMessage).toHaveBeenCalledWith(8,
      expect.objectContaining({ type: 'clf-tab-close-check' }), { documentId: 'duplicate' }));
    if (scenario === 'keeper-closed') h.tabs.splice(0, 1);
    else if (scenario === 'moved-to-personal') h.tabs[1]!.windowId = 3;
    else { h.tabs[0]!.active = false; h.tabs[1]!.active = true; }
  } finally { release(); await retirement; }
  expect(h.remove).not.toHaveBeenCalled();
});

it('a wake revokes a pending close before its newer status response returns', async () => {
  const h = await worker([]);
  h.tabs.push({ id: 7, url: `https://chatgpt.com/c/${firstId}` });
  await h.authorizeDocument({ tab: { id: 7 }, documentId: 'retired', frameId: 0, url: h.tabs[0]!.url }, { navigationEpoch: 1 });
  let releaseProof!: () => void, releaseStatus!: () => void;
  const proofHeld = new Promise<void>(resolve => { releaseProof = resolve; });
  const statusHeld = new Promise<void>(resolve => { releaseStatus = resolve; });
  let newerStatus = false, statusReached = false;
  h.fetch.mockImplementation(async input => {
    if (new URL(input).pathname === '/hello') return { ok: true, status: 200,
      json: async () => ({ app: 'chat-on-steroids', bridge: BRIDGE_PROTOCOL, compatible: true, paired: true }) };
    if (new URL(input).pathname === '/status' && newerStatus) { statusReached = true; await statusHeld; }
    return { ok: true, status: 200, json: async () => ({ ok: true, inputs: [], managedConversations: [firstId],
      retiredConversations: [firstId], nonDiscardableConversations: newerStatus ? [firstId] : [] }) };
  });
  h.sendMessage.mockImplementation(async (_tab, message): Promise<any> => {
    if (message.type === 'clf-tab-close-check') await proofHeld;
    return { safe: true, conversationId: firstId, navigationEpoch: 1, ok: true, recorderVersion: 11 };
  });
  const firstPass = h.maintain();
  let nextPass: Promise<void> | undefined;
  try {
    await vi.waitFor(() => expect(h.sendMessage).toHaveBeenCalledWith(7,
      expect.objectContaining({ type: 'clf-tab-close-check' }), { documentId: 'retired' }));
    newerStatus = true;
    nextPass = h.maintain(true);
    await vi.waitFor(() => expect(statusReached).toBe(true));
    releaseProof(); await h.settleRetirements();
    expect(h.remove).not.toHaveBeenCalled();
  } finally { releaseProof(); releaseStatus(); await firstPass; await nextPass; await h.settleRetirements(); }
  expect(h.remove).not.toHaveBeenCalled();
});


it('reveals only the explicitly selected existing worker chat, without opening another tab', async () => {
  const h = await worker([]);
  h.tabs.push({ id: 1, windowId: 80, url: `https://chatgpt.com/c/${firstId}` },
    { id: 2, windowId: 80, url: `https://chatgpt.com/c/${secondId}` },
...
it.each(['absent', 'duplicate', 'mixed-window', 'navigated'])('fails exact reveal closed for %s without an opener fallback', async reason => {
  const h = await worker([]);
  h.tabs.push({ id: 1, windowId: 80, url: `https://chatgpt.com/c/${firstId}` });
  h.saved.chatBackgroundWindow = 80; h.saved.chatBackgroundTabs = [1];
...
it('reports a refused isolated worker window immediately and never falls back to a personal tab', async () => {
  const h = await worker([]);
  h.tabs.push({ id: 20, windowId: 3, url: 'https://example.com/personal', active: true });
  h.windows.create.mockRejectedValueOnce(new Error('unavailable'));
  h.fetch.mockImplementation(async input => ({ ok: true, status: 200, json: async () =>
    new URL(input).pathname === '/hello' ? { app: 'chat-on-steroids', bridge: BRIDGE_PROTOCOL, compatible: true, paired: true }
    : { ok: true, inputs: [], background: false, placement: { id: firstId, background: true } } }));
  await h.maintain();
  expect(h.create).not.toHaveBeenCalled();
  expect(h.update).not.toHaveBeenCalled();
  expect(h.fetch.mock.calls.some(([url]) => new URL(url).pathname === '/commands/background-failed')).toBe(true);
});
...
it('keeps active personal browsing untouched for a new desktop chat', async () => {
  ...
  h.fetch.mockImplementation(async input => ({ ok: true, status: 200, json: async () =>
    new URL(input).pathname === '/hello' ? { app: 'chat-on-steroids', bridge: BRIDGE_PROTOCOL, compatible: true, paired: true }
    : { ok: true, inputs: [{ id: firstId, conversationId }], background: false } }));
  await h.maintain(); await h.maintain();
  expect(h.windows.create).toHaveBeenCalledExactlyOnceWith(expect.objectContaining({ state: 'minimized', focused: false }));
  expect(h.tabs.find(tab => tab.id === 30)).toMatchObject({ windowId: 3, active: true, url: personalUrl });
  expect(h.sendMessage.mock.calls.some(([id]) => id === 30)).toBe(false);
...
it('rechecks native isolation for both desktop input claim and final Send authorization', async () => {
  const h = await worker([]);
  const tab = await h.createChatTab(`https://chatgpt.com/c/${secondId}`, true);
  tab.url = tab.pendingUrl; delete tab.pendingUrl;
  const sender = { tab: { id: tab.id }, documentId: 'exact-document', frameId: 0, url: tab.url };
  const source = await h.authorizeDocument(sender, { navigationEpoch: 1 });
  await h.desktopInput({ id: firstId, conversationId: secondId, requiresAuthorization: true }, sender, source);
  tab.windowId = 3;
  await h.desktopInput({ id: firstId, conversationId: secondId, isolated: true, authorize: true }, sender, source);
  const claims = h.fetch.mock.calls.filter(([url]) => new URL(url).pathname === '/input/claim').map(([, init]) => JSON.parse(String(init?.body)));
  expect(claims).toHaveLength(2);
  expect(claims[0]).toMatchObject({ isolated: true, requiresAuthorization: true });
  expect(claims[1]).toMatchObject({ isolated: false, authorize: true });
});


it('plugin refresh creates an isolated minimized helper without background preference or personal-tab changes', async () => {
  const h = await worker([]);
  const personal = { id: 20, windowId: 3, url: 'https://example.com/personal', active: true };
  h.tabs.push(personal);
  h.fetch.mockImplementation(async input => ({ ok: true, status: 200, json: async () =>
    new URL(input).pathname === '/hello' ? { app: 'chat-on-steroids', bridge: BRIDGE_PROTOCOL, compatible: true, paired: true }
      : { requests: [{ id: firstId }] } }));
  await (h as any).inspectRequestedPluginRefresh([{ id: firstId }], false);
  expect(h.windows.create).toHaveBeenCalledExactlyOnceWith(expect.objectContaining({ state: 'minimized', focused: false }));
  expect(h.update).not.toHaveBeenCalled();
  expect(h.windows.update).not.toHaveBeenCalled();
  expect(personal).toEqual({ id: 20, windowId: 3, url: 'https://example.com/personal', active: true });
});

it('plugin refresh refuses a saved helper moved into a personal window without navigation or another tab', async () => {
  const h = await worker([]);
  h.tabs.push({ id: 20, windowId: 3, url: 'https://chatgpt.com/#settings/Plugins', active: true });
  h.saved.pluginRefreshOwner = { id: firstId, tab: 20 };
  h.fetch.mockImplementation(async input => ({ ok: true, status: 200, json: async () =>
    new URL(input).pathname === '/hello' ? { app: 'chat-on-steroids', bridge: BRIDGE_PROTOCOL, compatible: true, paired: true }
      : { requests: [{ id: firstId }] } }));
  await (h as any).inspectRequestedPluginRefresh([{ id: firstId }], false);
  ...
  expect(h.create).not.toHaveBeenCalled();
  expect(h.sendMessage).not.toHaveBeenCalled();
  expect(h.fetch.mock.calls.some(([, init]) => init?.body && JSON.parse(String(init.body)).action === 'fail')).toBe(true);
});

it('refuses automatic repair of a personal-copy conversation without physical app ownership', async () => {
  const h = await worker([]);
  const personal = { id: 20, windowId: 3, url: `https://chatgpt.com/c/${firstId}`, active: true };
  h.tabs.push(personal);
  h.fetch.mockImplementation(async input => ({ ok: true, status: 200, json: async () =>
    new URL(input).pathname === '/hello' ? { app: 'chat-on-steroids', bridge: BRIDGE_PROTOCOL, compatible: true, paired: true }
      : { ok: true, background: false, repairs: [{ conversationId: firstId, token: 'repair-token', focus: true }] } }));
  await h.maintain();
  expect(h.windows.create).not.toHaveBeenCalled();
  expect(h.reload).not.toHaveBeenCalled();
  expect(h.update).not.toHaveBeenCalled();
  ...
  expect(h.fetch.mock.calls.some(([url]) => new URL(url).searchParams.get('repairError') === 'BACKGROUND_UNAVAILABLE')).toBe(true);
});


it('model discovery opens an isolated helper without probing a personal ChatGPT document', async () => {
  const h = await worker([]);
  h.tabs.push({ id: 20, windowId: 3, url: `https://chatgpt.com/c/${firstId}`, active: true });
  await h.inspectModels({ nonce: secondId, expiresAt: Date.now() + 60000, allowOpen: true }, false);
  expect(h.windows.create).toHaveBeenCalledExactlyOnceWith(expect.objectContaining({ state: 'minimized', focused: false }));
  expect(h.sendMessage).not.toHaveBeenCalled();
  expect(h.update).not.toHaveBeenCalledWith(20, expect.anything());
  expect(h.tabs[0]).toEqual({ id: 20, windowId: 3, url: `https://chatgpt.com/c/${firstId}`, active: true });
});
