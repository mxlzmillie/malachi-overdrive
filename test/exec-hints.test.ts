...
describe('a non-zero exit that is a result rather than a failure', () => {
  ...
  it('binds bare PowerShell and POSIX ripgrep to the bundled executable without touching explicit/dynamic forms', () => {
    const bundled = 'C:\\Program Files\\MALACHI OVERDRIVE\\resources\\rg\\rg.exe';
    expect(bindBundledRipgrep('rg -n foo src', 'powershell', bundled)).toBe(
      "& 'C:\\Program Files\\MALACHI OVERDRIVE\\resources\\rg\\rg.exe' -n foo src"
    );
    expect(bindBundledRipgrep('rg foo | Select-Object -First 5', 'powershell', bundled)).toBe(
      "& 'C:\\Program Files\\MALACHI OVERDRIVE\\resources\\rg\\rg.exe' foo | Select-Object -First 5"
    );
    expect(bindBundledRipgrep('Write-Output x; ripgrep foo', 'powershell', bundled)).toBe(
      "Write-Output x; & 'C:\\Program Files\\MALACHI OVERDRIVE\\resources\\rg\\rg.exe' foo"
    );
    ...
    const posix = '/Applications/MALACHI OVERDRIVE/resources/rg/rg';
    const boundPosix = "'/Applications/MALACHI OVERDRIVE/resources/rg/rg' foo";
