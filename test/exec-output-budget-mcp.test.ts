...
afterEach(async () => {
  ...
  endpoint = null;
  // The HTTP response can finish before its recording/metadata writes. Resetting the
  // maps does not join those queues; deleting first races them and can cause ENOTEMPTY.
  await flushRecorder();
  await flushSessions();
  await flushDurable();
