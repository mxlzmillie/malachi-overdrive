...
describe('opening a chat on a goal', () => {
  it('does not start a reserved Goal provider request after recording authority is withdrawn', async () => {
    const session = await createSession({ title: 'Reserved Goal', conversationId: 'recording-race-goal' });
    const reserved = goal.startGoalDraft({
      conversationId: session.conversationId!,
      sessionId: session.id,
      turnId: 'recording-race-turn',
      deferStart: true
    });
    const config = defaultConfig();
    await saveConfig({ ...config, sessions: { ...config.sessions, record: false } });
    const request = vi.fn(async () => decision('continue', 'this must never be requested'));
    globalThis.fetch = request as never;

    expect(goal.beginGoalDraft(session.conversationId!, reserved.token)).toBe(false);
    expect(goal.goalViewFor(session.conversationId!)).toBeNull();
    expect(request).not.toHaveBeenCalled();
  });

  it('does not generate an opening message while session recording is off', async () => {
    const config = defaultConfig();
    await saveConfig({ ...config, sessions: { ...config.sessions, record: false } });
    const request = vi.fn(async () => decision('continue', 'this must never be requested'));
    globalThis.fetch = request as never;

    expect(await goal.draftOpeningMessage('Implement the change')).toEqual({ error: 'goal_disabled' });
    expect(request).not.toHaveBeenCalled();
  });
