...
it('completes only an explicitly temporary planner over HTTP without inventing a conversation id', async () => {
  ...
  const claimed = (await post('/input/claim', { id: row.id, owner: 'temp-page', conversationId: null, requiresAuthorization: true })).body.input;
  expect(claimed.lifetime).toBe('temporary-planner');
  expect(claimed.text).toBe('Transient plan context');
  expect(claimed.text).not.toContain(CODING_AGENT_TRANSPORT_CONTRACT);
...
describe('IPC input delivery and Goal control integration', () => {
  it.each([3, 5])('adds the shared %s-minute finish instruction to Astra opening input and excludes other models', async lead => {
    ...
    expect(claimed?.text).toBe(codingAgentDeliveryText(request.text) + '\n\n' + finishInstruction(lead));
    ...
    expect((await input.claimBrowserInput(ordinary.id, 'disabled', null))?.text).toBe(codingAgentDeliveryText(ordinary.text));
    ...
    expect((await input.claimBrowserInput(sol.id, 'sol', null))?.text).toBe(codingAgentDeliveryText(sol.text));
    ...
    expect((await input.claimBrowserInput(later.id, 'later', chat.conversationId))?.text).toBe(codingAgentDeliveryText(later.text));
  ...
  it('requires an exact plugin claim and matching schema before a refresh completion', async () => {
    ...
    publishPluginSurface('core', 'MALACHI OVERDRIVE Core', 'test', 'Synthetic instructions', tools);
    ...
    expect((await post('/plugin-refresh', { ...identity, action: 'claim', connectorName: 'MALACHI OVERDRIVE Core', tools })).body.ok).toBe(false);
    expect((await post('/plugin-refresh', { ...identity, action: 'claim', connectorName: 'MALACHI OVERDRIVE Core', tools: [{ ...tools[0], description: 'Old declaration' }] })).body.ok).toBe(true);
  ...
  it('defaults automatic plugin refresh off and revokes an already offered claim without removing the backend', async () => {
    ...
    plugin.publishPluginSurface('core', 'MALACHI OVERDRIVE Core', 'test', '', tools);
    ...
    const claim = { action: 'claim', id: saved.id, appId: 'asdk_app_off_on_test', connectorName: 'MALACHI OVERDRIVE Core', tools: [{ ...tools[0], description: 'Older declaration' }] };
  ...
  it('accepts a manual plugin-refresh terminal state and removes it from browser pickup', async () => {
    ...
    const manual = await post('/plugin-refresh', { ...request, appId: 'asdk_app_synthetic', action: 'manual', connectorName: 'MALACHI OVERDRIVE Core', tools: installed, error: 'Recreate or republish this custom app.' });
  ...
  it.each(['browser', 'tool'] as const)('records %s receipt text and pixels through the real IPC hook', async (transport) => {
    ...
    expect(rows[0]).toMatchObject({
      ...
      message: { text: codingAgentDeliveryText(authored.text) }
    });
  ...
  it('prepares fresh offline Goal before global activation and preserves authored enqueue identity', async () => {
    ...
    expect(claimed?.text).toBe(codingAgentDeliveryText(request.text, { terminalInstruction: GOAL_MARKER_INSTRUCTION }));
    expect(claimed?.text.endsWith(GOAL_MARKER_INSTRUCTION)).toBe(true);
    ...
    for (const automation of ['off', 'loop'] as const) {
      ...
      expect((await input.claimBrowserInput(manual.id, automation, null))?.text).toBe(codingAgentDeliveryText(manual.text));
  ...
  it('prepares scheduled tool input with the delivery backend and freezes retries across backend changes', async () => {
    ...
    try {
      ...
      expect(offered[0]?.text).toContain(request.text);
      expect(offered[0]?.text).toContain(CODING_AGENT_TRANSPORT_CONTRACT);
      expect(offered[0]?.text.endsWith(GOAL_MARKER_INSTRUCTION)).toBe(true);
      await saveConfig(defaultConfig());
      input.resetInputForTests();
...
it('delivers configured standing instructions as attributed context while retaining authored text', async () => {
  const config = defaultConfig();
  const standing = 'Use pnpm and run focused tests before reporting completion.';
  await saveConfig({ ...config, mcp: { ...config.mcp, instructions: standing } });
  const request = message(null, 'off');
  await input.enqueueInput(request as Parameters<typeof input.enqueueInput>[0]);
  const claimed = await input.claimBrowserInput(request.id, 'standing-instructions', null);
  expect(claimed?.text).toBe(codingAgentDeliveryText(request.text, { standingInstructions: standing }));
  expect(claimed?.text).toContain(`${STANDING_INSTRUCTIONS_HEADING}\n${standing}`);
  expect((await input.listInputs()).find(row => row.id === request.id)?.text).toBe(request.text);
  await input.cancelInput(request.id);
  await saveConfig(config);
});
