...
it('Record Off retires in-flight Goal work even when the app-wide Goal switch was already off', async () => {
  const goal = await import('../src/main/goal.js');
  const base = defaultConfig();
  await saveConfig({
    ...base,
    sessions: { ...base.sessions, record: true },
    // Per-chat Goal state may still be armed while this app-wide switch is already Off.
    goal: { ...base.goal, enabled: false }
  });
  const retired = vi.spyOn(goal, 'retireGoalDrafts');
  try {
    const current = getConfig();
    const result = await save({ ...current, sessions: { ...current.sessions, record: false } }, current);
