...
import { stopPreviews } from '../src/main/preview-server.js';
...
beforeAll(async () => {
  ...
  await writeTree(approved, {
    'site/index.html': '<h1>Preview fixture</h1>\n',
...
describe('surface boundaries', () => {
  ...
  it('advertises exactly Core’s tools on Core, with nothing from Desktop', async () => {
    ...
    expect(names).toEqual(['agents', 'apply_patch', 'download_artifact', 'exec_command', 'preview', 'read', 'session', 'view_image', 'write_stdin']);
  ...
  it('keeps the worst-case no-query discovery of each surface small', async () => {
    ...
    // Counts are the design for this capability-max fixture: first-class preview raises
    // Core to nine, find and the exec pair remain mutually exclusive, and Desktop is two.
    ...
    // surface measures today (core 15.3k, desktop 7.7k on 2026-09-09) rather than at a
  ...
  it('describes both surfaces well enough for a user to set them up and a model to find them', () => {
    for (const surface of SURFACE_LIST) {
      expect(surface.serverName, surface.id).toMatch(/^malachi-overdrive-/);
      expect(surface.connectorName, surface.id).toContain('MALACHI OVERDRIVE');
...
describe('2025-era clients', () => {
  it('answers the initialize handshake', async () => {
    ...
    expect(reply.body.result.serverInfo.name).toBe('malachi-overdrive-core');
...
describe('capability gating', () => {
  it('starts a static preview inside the app and returns its short local URL', async () => {
  ...
  it('hides every writing and running tool in read-only mode', async () => {
    ...
    expect(toolNames(await core('tools/list'))).toEqual(['find', 'preview', 'read', 'view_image']);
  });
...
describe('exec sessions belong to the chat that opened them', () => {
  ...
  it('refuses write_stdin from a chat that does not own the session, and keeps serving the one that does', async () => {
    ...
    expect(textOf(stranger)).toContain(
      `write_stdin failed: session ${sessionId} is not proven to belong to this durable MALACHI OVERDRIVE session.`
    );
    ...
    expect(textOf(unproven)).toContain('is not proven to belong to this durable MALACHI OVERDRIVE session');
  ...
  it('does not let a stale owner inherit a recycled process id during the new exec yield', async () => {
    ...
    try {
      ...
      expect(textOf(stolen)).toContain('is not proven to belong to this durable MALACHI OVERDRIVE session');
