...
function page() {
  dom = new JSDOM('<section role="tabpanel" aria-labelledby="settings-trigger-Plugins"><h2>MALACHI OVERDRIVE Core</h2><button id="schema">Schema kopieren</button><footer><button id="refresh">Aktualisieren</button></footer></section>', { runScripts: 'outside-only', url: 'https://chatgpt.com/#settings/Plugins/plugin_asdk_app_synthetic' });
  ...
  const props = { connector: { id: 'asdk_app_synthetic', name: 'MALACHI OVERDRIVE Core', app_metadata: { version_id: 'asdk_app_v_synthetic' }, owners: ['never-copy'] },
...
it('reads localized installed declarations and the unique native refresh action from provider state', async () => {
  const { api } = page(); const view = await api.pluginRefreshView('MALACHI OVERDRIVE Core', [tool]);
...
it('does not substitute expected declarations for changed provider descriptions', async () => {
  ...
  expect((await api.pluginRefreshView('MALACHI OVERDRIVE Core', [tool])).tools[0].description).toBe('Changed declaration.');
  props.actions.push(props.actions[0]!);
  expect(await api.pluginRefreshView('MALACHI OVERDRIVE Core')).toBeNull();
});
it('uses an enrolled App ID through renames, but refuses mismatched IDs and loading schemas', async () => {
  ...
  expect(await api.pluginRefreshView('MALACHI OVERDRIVE Core')).toBeNull();
  expect(await api.pluginRefreshView('MALACHI OVERDRIVE Core', [], 'asdk_app_synthetic')).not.toBeNull();
  expect(await api.pluginRefreshView('MALACHI OVERDRIVE Core', [], 'asdk_app_other')).toBeNull();
...
it('refuses ambiguous native actions and never copies unrelated connector properties', async () => {
  ...
  await api.pluginRefreshView('MALACHI OVERDRIVE Core'); expect(JSON.stringify(messages)).not.toContain('never-copy');
...
it('discovers exact installed rows across languages and preserves ambiguity', () => {
  ...
  expect(api.pluginInstalledButtons('MALACHI OVERDRIVE Core')).toBeNull();
  panel.insertAdjacentHTML('beforeend', '<button><span data-testid="plugin-icon-wrapper"></span><div>MALACHI OVERDRIVE Core</div><span>Alle zulassen</span></button>');
  expect(api.pluginInstalledButtons('MALACHI OVERDRIVE Core')).toHaveLength(1);
  expect(api.pluginInstalledButtons('MALACHI OVERDRIVE Desktop')).toEqual([]);
  panel.insertAdjacentHTML('beforeend', panel.querySelector('button')!.outerHTML);
  expect(api.pluginInstalledButtons('MALACHI OVERDRIVE Core')).toHaveLength(2);
});
...
it('observes an exact installed card without a refresh action through the real Fiber bridge', async () => {
  ...
  refresh.remove();
  expect(await api.pluginRefreshView('MALACHI OVERDRIVE Core', [tool])).toMatchObject({ appId: 'asdk_app_synthetic', tools: [tool], refresh: null });
  delete (dom.window.document.getElementById('schema') as any).__reactFiber$fixture.return;
  expect(await api.pluginRefreshView('MALACHI OVERDRIVE Core', [tool])).toBeNull();
});
