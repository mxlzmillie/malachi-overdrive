...
const publish = (version = '1', declarations = tools) => { publishPluginSurface('core', 'MALACHI OVERDRIVE Core', version, 'Instructions', declarations); vi.advanceTimersByTime(20_000); };
const claim = (request: { id: string }, declarations = [{ ...tools[0]!, description: 'Older declaration' }]) => claimPluginRefresh({ ...request, appId, connectorName: 'MALACHI OVERDRIVE Core', tools: declarations });
it('debounces only changed declarations for twenty seconds and fences stale claims', async () => {
  ...
  const change = (description: string) => publishPluginSurface('core', 'MALACHI OVERDRIVE Core', '1', 'Instructions', [{ ...tools[0]!, description }]);
  change('A');
...
it('keeps Desktop independent and debounces shape changes across reconnects', async () => {
  ...
  publishPluginSurface('core', 'MALACHI OVERDRIVE Core', '1', '', [{ ...tools[0]!, description: 'Reconnected shape' }]);
...
it('wakes a settled publication restored after its deadline elapsed while disconnected', async () => {
  ...
  publishPluginSurface('core', 'MALACHI OVERDRIVE Core', '1', '', changed);
  unpublishPluginSurface('core');
  vi.advanceTimersByTime(20_000);
...
it('enrolls already-current tools without granting a refresh click, including after restart', async () => {
  ...
  expect(await claimPluginRefresh({ ...request, appId, connectorName: 'MALACHI OVERDRIVE Core', tools, alreadyCurrent: true })).toBe(true);
  expect(await pendingPluginRefreshes()).toEqual([]);
  resetPluginRefreshForTests();
  publishPluginSurface('core', 'MALACHI OVERDRIVE Core', 'a-new-app-version', 'Different runtime instructions', tools);
  expect(await pendingPluginRefreshes()).toEqual([]);
});
it('does not settle a changed contract as already current', async () => {
  publish(); const request = (await pendingPluginRefreshes())[0]!;
  expect(await claimPluginRefresh({ ...request, appId, connectorName: 'MALACHI OVERDRIVE Core', tools: [{ ...tools[0]!, description: 'Old' }], alreadyCurrent: true })).toBe(false);
  expect(await claim(request)).toBe(true);
});
it('durably suppresses automatic retries when the provider requires manual recreation', async () => {
  ...
  expect(await requireManualPluginRefresh({ ...request, appId, connectorName: 'MALACHI OVERDRIVE Core', tools: installed, error: 'Recreate the custom app manually.' })).toBe(true);
  expect(await pendingPluginRefreshes()).toEqual([]);
...
it('keeps an exact app mapping and refuses another installed same-name plugin', async () => {
  ...
  expect(await claimPluginRefresh({ ...b, appId: 'asdk_app_other', connectorName: 'MALACHI OVERDRIVE Core', tools })).toBe(false);
});
...
it('requires readable declarations before claiming even an enrolled exact app', async () => {
  ...
  const next = (await pendingPluginRefreshes())[0]!;
  expect(await claimPluginRefresh({ ...next, appId, connectorName: 'MALACHI OVERDRIVE Core', tools: null })).toBe(false);
  expect((await readDurable('plugin-refresh') as any[])[0].attempted).toBe(false);
