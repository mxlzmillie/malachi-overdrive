...
it('routes exact plugin and tool identity for disabling a discovered tool', async () => {
  ...
  expect(document.querySelector('.toast')!.textContent).toContain('Refresh the MALACHI OVERDRIVE Plugins connector in ChatGPT');
});

it('distinguishes a serving connector from evidence of ChatGPT contact', () => {
  applyPluginsState({ config: { tunnel: { kind: 'manual' } }, status: { surfaces: [{ id: 'plugins', state: 'live', connectorName: 'MALACHI OVERDRIVE Plugins', description: 'Plugins', localUrl: 'http://localhost/mcp', lastRequestAt: null }] } } as unknown as AppState);
...
it('keeps first-use setup and the connector-refresh instruction visible, including after connection', async () => {
  ...
  expect(document.querySelector('.plugin-refresh-guide')!.textContent).toContain('refresh MALACHI OVERDRIVE Plugins in ChatGPT');
...
it('keeps plugin connection setup local, preserves a draft and saves through the existing settings authority', async () => {
  const next = { hasApiKey: true, config: { tunnel: { kind: 'openai', tunnelId: 'core-original', pluginsTunnelId: 'plugins-original' }, ui: { theme: 'dark' } }, status: { surfaces: [{ id: 'plugins', state: 'live', tools: ['get_scene_info'], connectorName: 'MALACHI OVERDRIVE Plugins', description: 'External tools', lastRequestAt: 1 }] } } as unknown as AppState;
