...
it('preserves the native browser selection for Work without inventing a Chat Pro override', async () => {
  dom = new JSDOM(await readFile('src/renderer/index.html', 'utf8'));
  vi.stubGlobal('window', dom.window); vi.stubGlobal('document', dom.window.document);
  const models = [{ id: '6', label: 'GPT-6 Pro', efforts: ['pro'] }];
  Object.assign(dom.window, { api: { getChatModels: async () => ({ ok: true, data: {
    state: 'ready', requestedAt: 1, observedAt: 2, models
  } }) } });
  const { initChatModels, applyChatModels, applyComposerSessionModel, confirmedComposerModel, ensureComposerModel } = await import('../src/renderer/chat-models.js');
  initChatModels(); applyChatModels({ multiAgent: {}, goal: {} } as Config); await Promise.resolve();
  applyComposerSessionModel('instagram-task', { model: 'gpt-6-astra-wm', reasoningEffort: 'medium', observedAt: 3 });
  expect(confirmedComposerModel()).toEqual({ model: null, reasoningEffort: null });
  expect(await ensureComposerModel()).toEqual({ model: null, reasoningEffort: null });
  expect(dom.window.document.getElementById('composerModelLabel')!.textContent).toBe('Current browser model');
  const status = dom.window.document.getElementById('composerModelStatus')!;
  expect(status.hidden).toBe(false);
  expect(status.textContent).toContain('Last recorded: gpt-6-astra-wm · Medium');
  expect(status.textContent).toContain('which may differ');
  expect(status.textContent).toContain('Change the model and thinking effort in ChatGPT');
  expect(dom.window.document.querySelector('#composerPowerChoices input')).toBeNull();
  const select = dom.window.document.getElementById('composerModel') as HTMLSelectElement;
  expect([...select.options].map(option => option.text)).toEqual(['Current browser model']);
  const injected = dom.window.document.createElement('option'); injected.value = '6'; select.append(injected); select.value = '6';
  const effort = dom.window.document.getElementById('composerReasoning') as HTMLSelectElement;
  const injectedEffort = dom.window.document.createElement('option'); injectedEffort.value = 'pro'; effort.append(injectedEffort); effort.value = 'pro';
  expect(confirmedComposerModel()).toBeNull();
  (dom.window.document.querySelector('[data-model-choice="current-browser"]') as HTMLButtonElement).click();
  expect(confirmedComposerModel()).toEqual({ model: null, reasoningEffort: null });
  applyComposerSessionModel('other-chat', { model: '6', reasoningEffort: 'pro', observedAt: 4 });
  ...
  (dom.window.document.querySelector('[data-model-choice="current-browser"]') as HTMLButtonElement).click();
  expect(confirmedComposerModel()).toEqual({ model: null, reasoningEffort: null });
  applyComposerSessionModel('other-chat', { model: '6', reasoningEffort: 'pro', observedAt: 5 });
  applyChatModels({ multiAgent: {}, goal: {} } as Config); await Promise.resolve();
  expect(confirmedComposerModel()).toEqual({ model: null, reasoningEffort: null });
  applyComposerSessionModel('instagram-task:2', { model: 'gpt-6-astra-wm', reasoningEffort: 'medium', observedAt: 3 });
  expect(confirmedComposerModel()).toEqual({ model: null, reasoningEffort: null });
  expect(dom.window.document.querySelector('#composerPowerChoices input')).toBeNull();
  applyComposerSessionModel(null, null);
  expect(confirmedComposerModel()).toEqual({ model: '6', reasoningEffort: 'pro' });
  expect(dom.window.document.querySelector('[data-model-choice="current-browser"]')).toBeNull();
});

it('allows an inherited browser choice immediately when discovery is unavailable but never carries it to a new chat', async () => {
  dom = new JSDOM(await readFile('src/renderer/index.html', 'utf8'));
  vi.stubGlobal('window', dom.window); vi.stubGlobal('document', dom.window.document);
  const requestChatModels = vi.fn(async () => ({ ok: true, data: { state: 'unavailable', requestedAt: 1, observedAt: 2, models: [] } }));
  Object.assign(dom.window, { api: { requestChatModels } });
  const { initChatModels, applyComposerSessionModel, confirmedComposerModel, ensureComposerModel } = await import('../src/renderer/chat-models.js');
  initChatModels();
  applyComposerSessionModel('work:1', { model: 'gpt-6-astra-wm', observedAt: 1 });
  expect(await ensureComposerModel()).toEqual({ model: null, reasoningEffort: null });
  const menu = dom.window.document.getElementById('modelMenu') as HTMLDetailsElement;
  menu.open = true; await new Promise(resolve => setTimeout(resolve, 0));
...
it('does not reinterpret a recorded unsupported effort even when the exact model exists', async () => {
  dom = new JSDOM(await readFile('src/renderer/index.html', 'utf8'));
  vi.stubGlobal('window', dom.window); vi.stubGlobal('document', dom.window.document);
  ...
  const { initChatModels, applyChatModels, applyComposerSessionModel, confirmedComposerModel } = await import('../src/renderer/chat-models.js');
  initChatModels(); applyChatModels({ multiAgent: {}, goal: {} } as Config); await Promise.resolve();
  applyComposerSessionModel('recorded:1', { model: 'gpt-6-pro', reasoningEffort: 'medium', observedAt: 3 });
  expect(confirmedComposerModel()).toBeNull();
  (dom.window.document.querySelector('[data-model-choice="current-browser"]') as HTMLButtonElement).click();
  expect(confirmedComposerModel()).toEqual({ model: null, reasoningEffort: null });
});

it('uses recorded Work context policy when inheriting instead of treating its family name as Pro', async () => {
  dom = new JSDOM(await readFile('src/renderer/index.html', 'utf8'));
  vi.stubGlobal('window', dom.window); vi.stubGlobal('document', dom.window.document);
  const { paintContextMeter } = await import('../src/renderer/context-meter.js');
  const config = { sessions: { limitTokens: 533000 }, compaction: { auto: true, autoTokens: 400000 } } as Config;
  paintContextMeter({ conversationId: 'work-chat', contextTokens: 1000, selectedModel: {
    conversationId: 'work-chat', model: 'gpt-6-astra-wm', reasoningEffort: 'medium', observedAt: 1
  } } as any, config, { model: null, reasoningEffort: null });
  expect(dom.window.document.getElementById('contextMeterInfo')!.textContent).not.toContain('Auto-compaction off for Pro');
...
it('renders the two observed Pro generations separately and sends their exact selection identities', async () => {
  ...
  expect(dom.window.document.getElementById('composerModelLabel')!.textContent).toBe('GPT-6 Astra · Pro');
  expect(dom.window.document.getElementById('composerPowerModel')!.textContent).toBe('GPT-6 Astra');
  expect(slider.getAttribute('aria-valuetext')).toBe('GPT-6 Astra · Pro');
...
it('offers only observed models, prefers supported GPT-6 High, and replaces a removed account selection', async () => {
  ...
  expect(select('composerReasoning').value).toBe('pro');
  expect(dom.window.document.getElementById('composerPowerModel')!.textContent).toContain('GPT-6 Astra');
...
it('defaults a new chat to account-observed GPT-6 Pro even when Sol is the first catalog row', async () => {
  dom = new JSDOM(await readFile('src/renderer/index.html', 'utf8'));
  vi.stubGlobal('window', dom.window); vi.stubGlobal('document', dom.window.document);
  const models = [
    { id: '5.6', label: 'GPT-5.6 Sol', efforts: ['none', 'high', 'pro'] },
