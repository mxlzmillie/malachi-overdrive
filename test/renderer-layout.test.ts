...
describe('the window as a whole', () => {
  it('keeps the conversation scrollbar on the outside edge of the sidebar', () => {
    const sidebarScroll = rule('.sidebar-sessions > .scroll');
    expect(sidebarScroll).toContain('margin-right: -12px');
    expect(sidebarScroll).toContain('padding-right: 12px');
    expect(sidebarScroll).toContain('scrollbar-gutter: stable');
  });

  it('provides one bounded, labelled surface for plans and queued work', () => {
    expect(document.getElementById('finishQueue')?.getAttribute('aria-label')).toBe('Plans and queued tasks');
    expect(rule('#finishQueue')).toContain('max-height: 300px');
    expect(rule('#finishQueue')).toContain('overflow-y: auto');
    expect(rule('.plan-step.is-active')).toContain('background:');
  });
