...
it('preserves an organised project draft and attachments when returning from another project', async () => {
  const projectA: LocalProject = { id: 'work-a', name: 'Overdrive', path: '/repo/a', createdAt: 1, kind: 'work', brief: 'Custom source only.' };
  const projectB: LocalProject = { id: 'work-b', name: 'Second project', path: '/repo/b', createdAt: 2, kind: 'work' };
  const { w, live } = await boot([], false, [], [projectA, projectB]);
  const open = (id: string) => (w.document.querySelector(`[data-project-id="${id}"] .project-heading`) as HTMLElement).click();
  open(projectA.id);
  const input = w.document.getElementById('chatInput') as HTMLTextAreaElement;
  input.value = 'Keep my exact unfinished requirements.\n  Preserve this spacing.';
  input.dispatchEvent(new w.Event('input', { bubbles: true }));
  const attachment = { id: 'draft-file', name: 'requirements.md', size: 123, mimeType: 'text/markdown' };
  (w as any).api.chooseFiles = () => Promise.resolve({ ok: true, data: [attachment] });
  (w.document.getElementById('attachImages') as HTMLButtonElement).click();
  await settle();
  open(projectB.id);
  open(projectA.id);
  expect(input.value).toBe('Keep my exact unfinished requirements.\n  Preserve this spacing.');
  expect(w.document.getElementById('composerImages')!.textContent).toContain('requirements.md');
  expect(live.sent).toHaveLength(0);
});

it('opens the command centre on a real renderer and prepares a brief without sending or enabling automation', async () => {
  const work: LocalProject = { id: 'work-a', name: 'Overdrive', path: '/repo/a', createdAt: 1, kind: 'work', brief: 'Use the customised source.' };
  const { w, live } = await boot([], false, [], [work]);
  w.HTMLDialogElement.prototype.showModal = function () { this.open = true; };
  w.HTMLDialogElement.prototype.close = function () { this.open = false; };
  (w.document.getElementById('overdriveCommand') as HTMLButtonElement).click();
  (w.document.querySelector('[data-command="project:work-a"]') as HTMLElement).click();
  const input = w.document.getElementById('chatInput') as HTMLTextAreaElement;
  expect(input.value).toContain('Use the customised source.');
  expect(w.document.getElementById('workContext')!.hidden).toBe(false);
  expect(w.document.getElementById('workContextLabel')!.textContent).toBe('Overdrive');
  input.value = 'Preserve my manual requirements.';
  input.dispatchEvent(new w.Event('input', { bubbles: true }));
  (w.document.getElementById('workRecipes') as HTMLButtonElement).click();
  (w.document.querySelector('[data-command="recipe:fix"]') as HTMLElement).click();
  expect(input.value.startsWith('Preserve my manual requirements.')).toBe(true);
  expect(input.value).toContain('Diagnose and fix');
  expect(live.sent).toHaveLength(0);
  expect(live.controlCalls).toHaveLength(0);
  expect(live.automation).toBe('off');
});

it('resumes the correct project draft from the workbench and prepares a brief without sending', async () => {
  const work: LocalProject = { id: 'work-a', name: 'Overdrive', path: '/repo/a', createdAt: 1, kind: 'work', brief: 'Use the customised source.' };
  const { w, live } = await boot([], false, [], [work]);
  const project = () => w.document.querySelector<HTMLButtonElement>('[data-workbench-action="project"][data-workbench-id="work-a"]')!;
  expect(project()).not.toBeNull(); project().click();
  const input = w.document.getElementById('chatInput') as HTMLTextAreaElement;
  expect(input.value).toContain('Use the customised source.');
  input.value = 'Finish these exact requirements.'; input.dispatchEvent(new w.Event('input', { bubbles: true }));
  (w.document.getElementById('newChat') as HTMLButtonElement).click();
  expect(project().textContent).toContain('Resume draft'); project().click();
  expect(input.value).toBe('Finish these exact requirements.');
  w.document.querySelector<HTMLButtonElement>('[data-workbench-action="recipe"][data-workbench-id="verify"]')!.click();
  expect(input.value).toContain('Finish these exact requirements.');
  expect(input.value).toContain('Verify before release');
  expect(live.sent).toHaveLength(0); expect(live.controlCalls).toHaveLength(0);
});
...
it('edits a long queued task without a stale HTML limit and refuses oversized saves without losing text', async () => {
  const { w, live, append } = await boot([]);
  live.inputs.push({ id: 'long-task', sessionId: summary([]).id, text: 'x'.repeat(24000), mode: 'after-turn', dueAt: 0,
    model: null, reasoningEffort: null, state: 'queued', owner: null, createdAt: 0, conversationId: 'chat-b' });
  ...
  const save = field.nextElementSibling as HTMLButtonElement;
  field.value = 'a'.repeat(64001); save.click(); await settle();
  expect(edit).not.toHaveBeenCalled();
  expect(field.value).toHaveLength(64001);
  expect(field.getAttribute('aria-invalid')).toBe('true');
  field.value = 'a'.repeat(64000); field.dispatchEvent(new w.Event('input')); save.click(); await settle();
  expect(edit).toHaveBeenCalledWith('long-task', 'a'.repeat(64000));
});

it('restores a draft after rejected send IPC without an automatic second send', async () => {
  const { w } = await boot([], false);
  const send = vi.fn(async () => { throw new Error('IPC disconnected'); });
  (w as any).api.sendInput = send;
  const input = w.document.getElementById('chatInput') as HTMLTextAreaElement;
  input.value = 'Keep every word.\n  Including this spacing.';
  input.dispatchEvent(new w.Event('input', { bubbles: true }));
  w.document.getElementById('composer')!.dispatchEvent(new w.Event('submit', { bubbles: true, cancelable: true }));
  await settle(50);
  expect(input.value).toBe('Keep every word.\n  Including this spacing.');
  expect(send).toHaveBeenCalledTimes(1);
  expect(w.document.body.textContent).toContain('Could not confirm delivery');
});

it('keeps a rejected send draft in its original project when the user has switched projects', async () => {
  const projects: LocalProject[] = [
    { id: 'work-a', name: 'A', path: '/repo/a', createdAt: 1, kind: 'work' },
    { id: 'work-b', name: 'B', path: '/repo/b', createdAt: 2, kind: 'work' }
  ];
  const { w } = await boot([], false, [], projects);
  const open = (id: string) => w.document.querySelector<HTMLElement>(`[data-project-id="${id}"] .project-heading`)!.click();
  open('work-a');
  let reject!: (error: Error) => void;
  const send = vi.fn(() => new Promise((_resolve, failed) => { reject = failed; }));
  (w as any).api.sendInput = send;
  const input = w.document.getElementById('chatInput') as HTMLTextAreaElement;
  input.value = 'Project A original requirements.';
  input.dispatchEvent(new w.Event('input', { bubbles: true }));
  w.document.getElementById('composer')!.dispatchEvent(new w.Event('submit', { cancelable: true }));
  await settle(); open('work-b'); input.value = 'Project B newer requirements.';
  input.dispatchEvent(new w.Event('input', { bubbles: true }));
  reject(new Error('Connection interrupted')); await settle();
  expect(input.value).toBe('Project B newer requirements.');
  open('work-a'); expect(input.value).toBe('Project A original requirements.');
  expect(send).toHaveBeenCalledTimes(1);
});

it('does not replace a newer same-chat draft when an earlier send rejects', async () => {
  const { w } = await boot([], false);
  let reject!: (error: Error) => void;
  (w as any).api.sendInput = () => new Promise((_resolve, failed) => { reject = failed; });
  const input = w.document.getElementById('chatInput') as HTMLTextAreaElement;
  input.value = 'Original send'; input.dispatchEvent(new w.Event('input', { bubbles: true }));
  w.document.getElementById('composer')!.dispatchEvent(new w.Event('submit', { cancelable: true }));
  await settle(); input.value = 'Do not overwrite this new draft.';
  input.dispatchEvent(new w.Event('input', { bubbles: true }));
  reject(new Error('Connection interrupted')); await settle();
  expect(input.value).toBe('Do not overwrite this new draft.');
});

it('does not restore an already accepted message when a later automation control rejects', async () => {
  const { w, live } = await boot([], false);
  (w as any).api.sendInput = async (args: InputArgs) => {
    live.sent.push(args);
    const row: InputEntry = { ...args, automation: 'goal', state: 'queued', owner: null, createdAt: Date.now(), conversationId: null };
    live.inputs.push(row); return { ok: true, data: row };
  };
  (w as any).api.setInputAutomation = async () => { throw new Error('Control interrupted'); };
  const input = w.document.getElementById('chatInput') as HTMLTextAreaElement;
  input.value = 'Already accepted'; input.dispatchEvent(new w.Event('input', { bubbles: true }));
  w.document.getElementById('composer')!.dispatchEvent(new w.Event('submit', { cancelable: true }));
  await settle();
  expect(input.value).toBe(''); expect(live.sent).toHaveLength(1);
  expect(w.document.body.textContent).toContain('The message was queued, but a follow-up control could not be confirmed.');
});

it('cancels only the queued task edit without changing or removing its durable task', async () => {
  const { w, live, append } = await boot([]);
  live.inputs.push({ id: 'cancel-edit', sessionId: summary([]).id, text: 'Original queued task', mode: 'after-turn', dueAt: 0,
    model: null, reasoningEffort: null, state: 'queued', owner: null, createdAt: 0, conversationId: 'chat-b' });
  const edit = vi.fn(), cancel = vi.fn();
  (w as any).api.editQueuedInput = edit; (w as any).api.cancelInput = cancel;
  await append([]);
  w.document.querySelector<HTMLButtonElement>('[aria-label="Edit queued task"]')!.click();
  w.document.querySelector<HTMLTextAreaElement>('#finishQueue textarea')!.value = 'Unsubmitted edit';
  [...w.document.querySelectorAll<HTMLButtonElement>('#finishQueue button')].find(button => button.textContent === 'Cancel edit')!.click();
  expect(w.document.querySelector('#finishQueue textarea')).toBeNull();
  expect(w.document.querySelector('#finishQueue .queue-label')!.textContent).toBe('Original queued task');
  expect(edit).not.toHaveBeenCalled(); expect(cancel).not.toHaveBeenCalled();
});
...
it('renders a durable plan as complete, active, and future steps with progress', async () => {
  const { w, live, append } = await boot([]);
  const sessionId = summary([]).id;
  const planId = '00000000-0000-4000-8000-000000000111';
  const common = { sessionId, mode: 'finish' as const, dueAt: 0, model: null, reasoningEffort: null,
    owner: null, conversationId: 'chat-b', createdAt: 1 };
  live.inputs.push(
    { ...common, id: planId, text: 'Inspect the project', state: 'sent', deliveredAt: 2,
      plan: { id: planId, index: 0, total: 3 } },
    { ...common, id: '00000000-0000-4000-8000-000000000112', text: 'Implement the change', state: 'browser',
      plan: { id: planId, index: 1, total: 3 } },
    { ...common, id: '00000000-0000-4000-8000-000000000113', text: 'Verify the result', state: 'queued',
      plan: { id: planId, index: 2, total: 3 } }
  );
  await append([]);
  expect(w.document.querySelector('#finishQueue .plan-header-title')?.textContent).toBe('Plan');
  expect(w.document.querySelector('#finishQueue .plan-count')?.textContent).toBe('1 / 3');
  expect(w.document.querySelector('.plan-step.is-complete')?.textContent).toContain('Inspect the project');
  expect(w.document.querySelector('.plan-step.is-active')?.textContent).toContain('Implement the change');
  expect(w.document.querySelector('.plan-step.is-future')?.textContent).toContain('Verify the result');
  expect(w.document.querySelector('.plan-step.is-complete .plan-step-marker .ico')).not.toBeNull();
});
...
it('shows elapsed work for the exact recorded turn without exposing lifecycle rows', async () => {
  ...
  expect(w.document.getElementById('chatState')!.textContent).toContain('estimated finish');
  (w as any).api.getSessionControls = (id: string) => Promise.resolve({ ok: true, data: { sessionId: id, automation: 'off', activeTurnId: null, finishHeld: false, blocked: '', job: null } });
...
it('keeps an app-managed website preview visible as a compact link in the chat', async () => {
  const preview = toolCall(2, 'preview-call');
  if (preview.kind !== 'tool_call') throw new Error('fixture must be a tool call');
  preview.call.tool = 'preview';
  preview.call.result = text('Preview ready: [Open preview](http://127.0.0.1:41234/)');
  preview.call.summary = { kind: 'browse', title: 'Preview ready', tone: 'good', detail: 'command-center' };
  const { w } = await boot([toolCall(1, 'read-before'), preview, toolCall(3, 'read-after')]);
  const link = w.document.querySelector<HTMLAnchorElement>('.preview-link');
  expect(link?.textContent).toContain('Open preview');
  expect(link?.getAttribute('href')).toBe('http://127.0.0.1:41234/');
  expect(link?.closest('.tool-group')).toBeNull();
});
...
it('keeps editable stages and sends the original request with the full workflow exactly once', async () => {
  ...
  expect(w.document.querySelector('[aria-label="Plan step 2 of 2 · future"]')).not.toBeNull();
  expect(w.document.querySelector('#finishQueue .plan-header')?.textContent).toContain('Plan');
  expect(w.document.querySelector('#finishQueue .plan-count')?.textContent).toBe('0 / 2');
});
...
it.each([true, false])('hands plan presentation to queued stages while sending and restores a rejected draft (accepted=%s)', async accepted => {
  ...
  api.sendInput = vi.fn((args: InputArgs) => {
    ...
    // A queue observation can arrive before the original send IPC response. It must
    // not coexist with the editable six-stage draft (the screenshot's 6 + 5 rows).
    // The real outbox commits the source's stagesApplied receipt and child rows together.
    const plan = { id: first.id, index: 0, total: stages.length };
    live.inputs = [{ ...first, state: 'sent', stagesApplied: true, plan }, ...stages.slice(1).map((text, index) => ({ ...first,
      id: `stage-${index}`, text, mode: 'finish' as const, stages: undefined, plan: { ...plan, index: index + 1 } }))];
  ...
  expect(w.document.querySelectorAll('#finishQueue .queued-input')).toHaveLength(6);
  expect(w.document.querySelector('#finishQueue .plan-header')).not.toBeNull();
  expect(live.sent).toHaveLength(1);
