...
describe('durable user input ownership', () => {
  ...
  it('allows editing a long queued script and rejects oversize edits without losing it', async () => {
    const text = 'Long script line.\n'.repeat(2000);
    const row = await enqueueInput(input({ mode: 'finish', text }));
    const edited = text + 'Verify every page.';
    expect(await editQueuedInput(row.id, edited)).toBe(true);
    await expect(editQueuedInput(row.id, 'x'.repeat(64001))).rejects.toThrow();
    expect((await listInputs()).find(entry => entry.id === row.id)?.text).toBe(edited);
    await expect(enqueueInput(input({ sessionId: null, text: 'x'.repeat(64001) }))).rejects.toThrow();
  });
  ...
  it('retries preparation through existing offers but grants native Send only once to the current owner', async () => {
    ...
    now += 45000;
    expect((await listInputs())[0]).toMatchObject({ state: 'browser' });
    expect(await pendingBrowserInputs()).toEqual([]);
    expect(await authorizeBrowserInput(row.id, 'replacement-page', null)).toBe(false);
    now += 135000;
  ...
  it('keeps the pre-send claim live through the bounded model-selection window', async () => {
    const row = await enqueueInput(input({ sessionId: null }));
    await claimBrowserInput(row.id, 'page', null, true);
    now += 45000;
    expect(await failBrowserInput(row.id, 'page', 'Requested model could not be confirmed')).toBe(true);
    expect((await listInputs())[0]).toMatchObject({
      state: 'failed',
      error: 'Requested model could not be confirmed'
    });
  });
...
it('materializes remaining plan stages once after exact delivery, across restart', async () => {
  ...
  expect(await enqueueInput(first)).toMatchObject({ plan: { id: first.id, index: 0, total: 3 } });
  ...
  expect(initial.map(row => row.plan)).toEqual([
    { id: first.id, index: 1, total: 3 },
    { id: first.id, index: 2, total: 3 }
  ]);
...
it('migrates an exact legacy plan root before atomically materializing its children', async () => {
  const legacy = await seedLegacyInput(input({ sessionId: null, stages: ['Implement remaining work', 'Verify acceptance'] }));
  expect((await listInputs()).find(row => row.id === legacy.id)?.plan).toEqual({ id: legacy.id, index: 0, total: 3 });
  await claimBrowserInput(legacy.id, 'legacy-plan-tab', null);
  await acknowledgeBrowserInput(legacy.id, 'legacy-plan-tab', binding.conversationId);
  resetInputForTests();
  const plan = (await listInputs()).filter(row => row.plan?.id === legacy.id);
  expect(plan.map(row => row.plan?.index)).toEqual([0, 1, 2]);
  expect(plan.map(row => row.plan?.total)).toEqual([3, 3, 3]);
});

it('retains a completed plan prefix beyond the history cap while a later member is active', async () => {
  const planId = randomUUID();
  const root: InputEntry = { ...input({ text: 'Build the application', stages: ['Implement it', 'Verify it'] }),
    plan: { id: planId, index: 0, total: 3 }, stagesApplied: true, state: 'sent', owner: null,
    createdAt: 100, conversationId: binding.conversationId, historyRecorded: true };
  const completed: InputEntry = { ...input({ text: 'Implement it', mode: 'finish', dueAt: 100 }),
    plan: { id: planId, index: 1, total: 3 }, state: 'sent', owner: null,
    createdAt: 100, conversationId: binding.conversationId, historyRecorded: true };
  const active: InputEntry = { ...input({ text: 'Verify it', mode: 'finish', dueAt: 101 }),
    plan: { id: planId, index: 2, total: 3 }, state: 'queued', owner: null,
    createdAt: 101, conversationId: binding.conversationId };
  const unrelated = Array.from({ length: 55 }, (_, index): InputEntry => ({
    ...input({ text: `Old task ${index}`, mode: 'finish', dueAt: 200 + index }), state: 'sent', owner: null,
    createdAt: 200 + index, conversationId: binding.conversationId, historyRecorded: true
  }));
  await writeDurableNow('session-input', [root, completed, ...unrelated, active]);
  resetInputForTests();
  await enqueueInput(input({ text: 'Another checkpoint', mode: 'finish' }));
  resetInputForTests();
  const ids = new Set((await listInputs()).map(row => row.id));
  expect(ids.has(root.id)).toBe(true);
  expect(ids.has(completed.id)).toBe(true);
...
describe('Astra delivery boundaries and stacked direct input', () => {
  ...
  it('moves never-offered tool input to the browser after the activity lease expires', async () => {
    let active = true;
    binding.activeTurnId = 'orphaned-astra-turn';
    configureInputDelivery({ applyAutomation: automate, changed, hasActivity: () => active });
    const row = await enqueueInput(input({ text: 'Continue after the stalled answer' }));
    expect(row.transportIntent).toBe('tool');

    active = false;
    binding.end = { kind: 'turn_end', outcome: 'stalled', turnId: 'orphaned-astra-turn', time: now };
    resetInputForTests();
    const [migrated] = await listInputs();
    expect(migrated).toMatchObject({ id: row.id, state: 'queued', transportIntent: 'browser', reroutedAt: now });
    expect(await pendingBrowserInputs()).toEqual([{ id: row.id, conversationId: binding.conversationId }]);
  });
