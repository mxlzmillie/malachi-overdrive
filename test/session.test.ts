...
beforeEach(async () => {
  resetRecorderForTests();
  resetSessionStoreForTests();
  // Catalog recovery really writes the 5,001 synthetic checkpoints to disk. A
  // process-cache reset is not fixture isolation: sharing one directory forced
  // every later recorder case to rescan them, eventually timing out in a cascade.
  // Restart/recovery tests still retain their own files within the same case.
...
afterEach(async () => {
  // Recorder work can enqueue metadata writes after its caller returns. Join the
  // existing owners before changing roots/removing this case's disk fixture.
  await flushRecorder();
  await flushSessions();
  resetRecorderForTests();
  resetSessionStoreForTests();
...
describe('session store', () => {
  ...
  it('ends only the exact abandoned turn after bounded recovery is exhausted', async () => {
    const conversationId = 'c-abandoned-turn';
    const opened = await recordChatObservations(conversationId, [
      { kind: 'user_message', time: 10, text: 'run the long job', messageId: 'abandoned-user-1' },
      { kind: 'turn_start', time: 11, turnId: 'g-abandoned-open' }
    ]);
    const sessionId = opened.sessionId!;

    expect(await endAbandonedTurn(conversationId, sessionId, 'wrong-turn', 'stalled', 'expired')).toBe(false);
    expect(await endAbandonedTurn(conversationId, sessionId, 'g-abandoned-open', 'stalled', 'expired')).toBe(true);
    expect(await endAbandonedTurn(conversationId, sessionId, 'g-abandoned-open', 'stalled', 'expired')).toBe(false);

    expect(liveConversations().find(row => row.conversationId === conversationId)?.activeTurnId).toBeNull();
    expect(await getSession(sessionId)).toMatchObject({ activeTurnId: null, lastTurnOutcome: 'stalled' });
    expect(await readEvents(sessionId, { kinds: ['turn_end'] })).toEqual([
      expect.objectContaining({ source: 'app', turnId: 'g-abandoned-open', outcome: 'stalled', detail: 'expired' })
    ]);
  });
...
describe('canonical recorder 1.8', () => {
  it('starts recorder cases with an empty disk fixture after the large-catalog cases', async () => {
    // Resetting the process caches does not remove durable catalog rows. Those rows
    // must belong only to the test that created them, not every subsequent recorder case.
    const entries = await fs.readdir(sessionsRoot()).catch((error: NodeJS.ErrnoException) => {
    ...
    const opened = await recordChatObservations('conv-late', [
      {
        kind: 'user_message',
        time: Date.now(),
        text: 'You are worker agent "worker-1" in a MALACHI OVERDRIVE run.',
        messageId: 'boot-2'
      }
    ]);
...
describe('tool summaries', () => {
  ...
  it('presents a local preview as a ready website instead of a generic tool call', () => {
    expect(summarize('preview', { path: '/workspace/command-center' })).toMatchObject({
      kind: 'browse', tone: 'good', title: 'Preview ready', detail: 'command-center'
    });
  });
