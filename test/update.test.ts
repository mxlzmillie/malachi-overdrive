...
const previousReleaseRepository = process.env.MALACHI_OVERDRIVE_RELEASE_REPOSITORY;
process.env.MALACHI_OVERDRIVE_RELEASE_REPOSITORY = 'malachi-test/malachi-overdrive';
...
const NEXT = '99.0.0';
const WINDOWS_ASSET = `MALACHI-OVERDRIVE-Setup-${process.arch}.exe`;
const APPIMAGE_ASSET = `MALACHI-OVERDRIVE-Linux-${process.arch}.AppImage`;
...
afterAll(() => {
  if (previousReleaseRepository === undefined) delete process.env.MALACHI_OVERDRIVE_RELEASE_REPOSITORY;
  else process.env.MALACHI_OVERDRIVE_RELEASE_REPOSITORY = previousReleaseRepository;
});

describe('which installations update themselves', () => {
  it('takes the Windows installer and the Linux AppImage, and nothing else', () => {
    expect(stagedArtifact('win32', 'x64')).toMatchObject({ name: 'MALACHI-OVERDRIVE-Setup-x64.exe', kind: 'installer' });
    expect(stagedArtifact('linux', 'arm64', '/opt/cos.AppImage')).toMatchObject({
      name: 'MALACHI-OVERDRIVE-Linux-arm64.AppImage',
      kind: 'appimage',
...
describe('automatic restart admission', () => {
  const readyAt = 1_000_000;
  const idle = {
    stage: 'ready' as const,
    readySince: readyAt,
    now: readyAt + 5 * 60_000,
    systemIdleSeconds: 120,
    inFlightRequests: 0,
    generatingConversations: 0,
    backgroundProcesses: 0
  };

  it('admits only a settled downloaded update on an actually idle laptop', () => {
    expect(automaticInstallSafe(idle)).toBe(true);
    expect(automaticInstallSafe({ ...idle, stage: 'downloading' })).toBe(false);
    expect(automaticInstallSafe({ ...idle, readySince: null })).toBe(false);
    expect(automaticInstallSafe({ ...idle, now: readyAt + 5 * 60_000 - 1 })).toBe(false);
    expect(automaticInstallSafe({ ...idle, systemIdleSeconds: 119 })).toBe(false);
  });

  it('never restarts across browser, MCP or retained terminal work', () => {
    expect(automaticInstallSafe({ ...idle, inFlightRequests: 1 })).toBe(false);
    expect(automaticInstallSafe({ ...idle, generatingConversations: 1 })).toBe(false);
    expect(automaticInstallSafe({ ...idle, backgroundProcesses: 1 })).toBe(false);
  });
});
...
describe('staging the new version', () => {
  ...
  it('stages nothing when the release does not publish an artifact for this installation', async () => {
    github({ checksums: `${sha256('x')}  MALACHI-OVERDRIVE-Extension.zip\n` });
    await asPlatform('win32', undefined, () => checkForUpdates());
    expect(updateStatus().stage).toBe('failed');
  ...
  it('replaces the running AppImage with the staged one', async () => {
    const live = path.join(userData, 'MALACHI-OVERDRIVE.AppImage');
