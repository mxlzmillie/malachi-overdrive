...
describe('Windows login startup', () => {
  it('writes only packaged Windows login settings and supports turning the same entry off', () => {
    ...
    applyLoginStartup(app, true, 'win32', 'C:/Program Files/MALACHI OVERDRIVE/app.exe');
    applyLoginStartup(app, false, 'win32', 'C:/Program Files/MALACHI OVERDRIVE/app.exe');
    expect(app.setLoginItemSettings.mock.calls).toEqual([
      [{ openAtLogin: true, path: 'C:/Program Files/MALACHI OVERDRIVE/app.exe', args: ['--background'] }],
      [{ openAtLogin: false, path: 'C:/Program Files/MALACHI OVERDRIVE/app.exe', args: ['--background'] }]
    ]);
